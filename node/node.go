...
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, freezer, namespace string, readonly bool) (ethdb.Database, error) {
	...
	if n.config.DataDir == "" {
		...
		freezer = n.ResolveAncient(name, freezer)
		db, err = rawdb.NewLevelDBDatabaseWithFreezer(n.ResolvePath(name), cache, handles, freezer, namespace, readonly)
	}
...
// ResolveAncient returns the absolute path of an ancient store in the instance directory.
func (n *Node) ResolveAncient(name string, ancient string) string {
	root := n.ResolvePath(name)
	switch {
	case ancient == "":
		ancient = filepath.Join(root, "ancient")
