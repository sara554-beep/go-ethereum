...
// identifier, node path regardless what's the node hash. It's an internal
// function which assumes the lock is already held and content is not stale.
func (dl *diskLayer) nodeByPath(owner common.Hash, path []byte) []byte {
	// Try to retrieve the trie node from the dirty memory cache.
	// The map is lock free since it's impossible to mutate the
	// disk layer before tagging it as stale.
...
func (dl *diskLayer) Update(blockHash common.Hash, id uint64, nodes map[common.Hash]map[string]*memoryNode) *diffLayer {
...
func (dl *diskLayer) commit(bottom *diffLayer, force bool) (*diskLayer, error) {
	...
	ndl := newDiskLayer(bottom.root, bottom.id, dl.db, dl.dirty.commit(bottom.nodes))
