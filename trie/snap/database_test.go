...
func fill(n int, env *testEnv) {
	var (
		set      = make(map[string]*ntypes.NodeWithPrev)
		checkDup = func(path []byte) bool {
			if len(path) == 0 {
				return true
			}
			if _, ok := set[string(path)]; ok {
				return true
			}
			return false
		}
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			set[string(path)] = testutil.RandomNodeWithPrev(nil)
		case 1:
			// node modification
			...
			set[string(path)] = testutil.RandomNodeWithPrev(prev)
		case 2:
			// node deletion
			valid, path, prev := env.rand()
			if !valid || len(prev) == 0 {
			...
			set[string(path)] = ntypes.NewDeletedNodeWithPrev(prev)
		}
	}
	// Add the root node
	...
	env.db.Update(root.Hash, env.lastHash(), map[common.Hash]map[string]*ntypes.NodeWithPrev{{}: set})
...
func TestDatabaseRollback(t *testing.T) {
	...
	for i := index; i > 0; i-- {
		...
		for j := 0; j < len(paths); j++ {
			layer := env.db.GetReader(env.hashes[i-1])
			if len(blobs[j]) == 0 {
...
func TestDatabaseBatchRollback(t *testing.T) {
	...
	for i := 0; i < len(env.roots); i++ {
		_, exist := rawdb.ReadStateLookup(env.db.diskdb, env.hashes[i])
		if exist {
...
func TestJournal(t *testing.T) {
	...
	for i := index; i < len(env.hashes); i++ {
		...
		for j := 0; j < len(paths); j++ {
			...
			layer := newdb.GetReader(env.hashes[i])
			blob, err := layer.Node(common.Hash{}, paths[j], crypto.Keccak256Hash(blobs[j]))
			if err != nil {
				t.Error("Failed to retrieve state", "err", err)
			}
