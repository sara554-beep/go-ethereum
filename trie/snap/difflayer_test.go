...
func benchmarkSearch(b *testing.B, depth int, total int) {
	...
	fill := func(parent snapshot, index int) *diffLayer {
		nodes := make(map[common.Hash]map[string]*types.NodeWithPrev)
		nodes[common.Hash{}] = make(map[string]*types.NodeWithPrev)
		for i := 0; i < 3000; i++ {
			var (
				path = testutil.RandBytes(32)
				node = testutil.RandomNode()
			)
			nodes[common.Hash{}][string(path)] = types.NewNodeWithPrev(node.Hash, node.Blob, nil)
