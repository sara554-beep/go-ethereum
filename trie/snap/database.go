...
import (
	...
	ntypes "github.com/ethereum/go-ethereum/trie/types"
)
...
type snapshot interface {
	// nodeByPath retrieves the encoded trie node blob with the provided trie
	// identifier and node path regardless what's the node hash. No error will be
	// returned if the node is not found.
	nodeByPath(owner common.Hash, path []byte) ([]byte, error)

	// Node retrieves the encoded trie node blob with the provided trie identifier,
	// node path and the corresponding node hash. No error will be returned if the
	// node is not found.
	...
	// with the nil as the node object.
	//
	// Note, the maps are retained by the method to avoid copying everything.
	Update(blockRoot common.Hash, id uint64, nodes map[common.Hash]map[string]*ntypes.NodeWithPrev) *diffLayer
...
func (db *Database) Update(root common.Hash, parentRoot common.Hash, nodes map[common.Hash]map[string]*ntypes.NodeWithPrev) error {
