...
func (tree *layerTree) add(root common.Hash, parentRoot common.Hash, set map[common.Hash]map[string]*ntypes.NodeWithPrev) error {
...
func fixset(sets map[common.Hash]map[string]*ntypes.NodeWithPrev, layer snapshot) (map[common.Hash]map[string]*ntypes.NodeWithPrev, error) {
	var (
		hits int
		miss int
	)
	for owner, set := range sets {
		for path, n := range set {
			...
			set[path] = ntypes.NewNodeWithPrev(n.Hash, n.Blob, prev)
		}
	}
