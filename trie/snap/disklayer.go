...
func (dl *diskLayer) Update(blockHash common.Hash, id uint64, nodes map[common.Hash]map[string]*types.NodeWithPrev) *diffLayer {
...
func (dl *diskLayer) commit(bottom *diffLayer, force bool) (*diskLayer, error) {
	...
	slim := make(map[common.Hash]map[string]*types.Node)
	for owner, nodes := range bottom.nodes {
		subset := make(map[string]*types.Node)
