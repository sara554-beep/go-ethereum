...
func (db *Database) loadDiskLayer(r *rlp.Stream) (snapshot, error) {
	...
	var nodes = make(map[common.Hash]map[string]*ntypes.Node)
	for _, entry := range encoded {
		subset := make(map[string]*ntypes.Node)
		for _, n := range entry.Nodes {
			if len(n.Blob) > 0 {
				subset[string(n.Path)] = ntypes.NewNode(crypto.Keccak256Hash(n.Blob), n.Blob)
			} else {
				subset[string(n.Path)] = ntypes.NewDeletedNode()
			}
		}
...
func (db *Database) loadDiffLayer(parent snapshot, r *rlp.Stream) (snapshot, error) {
	...
	nodes := make(map[common.Hash]map[string]*ntypes.NodeWithPrev)
	for _, entry := range encoded {
		subset := make(map[string]*ntypes.NodeWithPrev)
		for _, n := range entry.Nodes {
			if len(n.Blob) > 0 {
				subset[string(n.Path)] = ntypes.NewNodeWithPrev(crypto.Keccak256Hash(n.Blob), n.Blob, n.Prev)
			} else {
				subset[string(n.Path)] = ntypes.NewDeletedNodeWithPrev(n.Prev)
			}
		}
		nodes[entry.Owner] = subset
	}
