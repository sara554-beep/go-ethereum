...
type diskcache struct {
	...
	nodes  map[common.Hash]map[string]*types.Node // The dirty node set, mapped by owner and path
}

// newDiskcache initializes the dirty node cache with the given information.
func newDiskcache(limit int, nodes map[common.Hash]map[string]*types.Node, layers uint64) *diskcache {
	// Don't panic for lazy callers.
	if nodes == nil {
		nodes = make(map[common.Hash]map[string]*types.Node)
	}
	var size uint64
...
func (cache *diskcache) node(owner common.Hash, path []byte, hash common.Hash) (*types.Node, error) {
...
// into the disk cache.
func (cache *diskcache) commit(nodes map[common.Hash]map[string]*types.Node) *diskcache {
	var (
		delta          int64
		overwrites     int64
...
func (cache *diskcache) revert(h *trieHistory) error {
	...
	for _, entry := range h.Tries {
		...
		for _, state := range entry.Nodes {
			...
			if len(state.Prev) == 0 {
				subset[string(state.Path)] = types.NewDeletedNode()
				delta -= int64(len(cur.Blob))
			} else {
				subset[string(state.Path)] = types.NewNode(crypto.Keccak256Hash(state.Prev), state.Prev)
...
func (cache *diskcache) reset() {
	cache.layers = 0
	cache.size = 0
	cache.nodes = make(map[common.Hash]map[string]*types.Node)
}
