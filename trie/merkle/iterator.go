...
type nodeIterator struct {
	...
	resolver trie.NodeResolver    // optional node resolver for avoiding disk hits
...
func newNodeIterator(trie *Trie, start []byte) trie.NodeIterator {
...
// of nodes seen.
func NewDifferenceIterator(a, b trie.NodeIterator) (trie.NodeIterator, *int) {
	a.Next(true)
...
func (it *differenceIterator) AddResolver(resolver trie.NodeResolver) {
...
type nodeIteratorHeap []trie.NodeIterator

func (h nodeIteratorHeap) Len() int            { return len(h) }
func (h nodeIteratorHeap) Less(i, j int) bool  { return compareNodes(h[i], h[j]) < 0 }
func (h nodeIteratorHeap) Swap(i, j int)       { h[i], h[j] = h[j], h[i] }
func (h *nodeIteratorHeap) Push(x interface{}) { *h = append(*h, x.(trie.NodeIterator)) }
...
func NewUnionIterator(iters []trie.NodeIterator) (trie.NodeIterator, *int) {
...
func (it *unionIterator) AddResolver(resolver trie.NodeResolver) {
	panic("not implemented")
}
...
func (it *unionIterator) Next(descend bool) bool {
	...
	least := heap.Pop(it.items).(trie.NodeIterator)

	// Skip over other nodes as long as they're identical, or, if we're not descending, as
	// long as they have the same prefix as the current node.
	for len(*it.items) > 0 && ((!descend && bytes.HasPrefix((*it.items)[0].Path(), least.Path())) || compareNodes(least, (*it.items)[0]) == 0) {
		skipped := heap.Pop(it.items).(trie.NodeIterator)
