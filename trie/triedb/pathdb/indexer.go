...
func newIndexer(disk ethdb.KeyValueStore, freezer *rawdb.ResettableFreezer, head uint64) *indexer {
	i := &indexer{
		...
		freezer: freezer,
		head:    head,
		rangeCh: make(chan historyRange),
...
func (i *indexer) notify(tail uint64, head uint64) error {
	...
	case i.rangeCh <- historyRange{tail: tail, head: head}:
	}
	return nil
}
...
func (i *indexer) next() (uint64, error) {
	...
	log.Debug("History gap detected, discard old segment", "oldHead", *head, "newHead", tail+1)
	return tail + 1, nil
}
...
func (i *indexer) run(done chan struct{}, head uint64, interrupt *atomic.Int32) {
	...
	for cur <= head {
		...
		s := time.Now()
		result, err := readHistories(i.freezer, cur, batch)
		if err != nil {
