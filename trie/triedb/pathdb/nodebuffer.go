...
// write. The content of the nodebuffer must be checked before the disk data
// (since it basically is not-yet-written data).
...
// commit merges the dirty nodes into the nodebuffer. This operation takes
// ownership of the nodes map(belongs to the bottom-most diff layer), and
// the caller must not change it afterwards.
...
// revert is the reverse operation of commit. It also merge the provided nodes
// into the nodebuffer, but difference is it decreases the layers counter.
func (b *nodebuffer) revert(nodes map[common.Hash]map[string]*trienode.Node) error {
	...
	for owner, subset := range nodes {
		...
		if !ok {
			panic(fmt.Sprintf("non-existent node (%x)", owner))
		}
		for path, n := range subset {
			cur, ok := current[path]
			if !ok {
...
func (b *nodebuffer) updateSize(delta int64) {
	...
	s := b.size
	b.size = 0
	log.Error("Invalid node buffer size", "prev", common.StorageSize(s), "delta", common.StorageSize(delta))
}
