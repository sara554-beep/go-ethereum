...
// provided nodes don't belong to any live layer which are generated inflight,
// safe to take the ownership if necessary.
	...
	for owner, subset := range nodes {
		...
		for path, n := range subset {
			...
			if !ok {
				...
				// a fullNode with only 2 children, and then a new child with
				// different position is immediately inserted into the fullNode.
				// In this case, the clean child will also be marked as dirty
				// because of node collapse and expansion.
				//
				// In case of database rollback, don't panic if this "clean"
				// node is occurred which was not present in nodeBuffer.
