...
func TestWriteIndex(t *testing.T) {
	...
	var (
		...
		accountsLast = make(map[common.Address][]byte)
		storages     = make(map[common.Address]map[common.Hash][]byte)
		storagesLast = make(map[common.Address]map[common.Hash][]byte)
	)
	for _, h := range hs {
	...
	for i, h := range hs {
		...
		for account, exp := range h.accounts {
			data, err := r.read(account, common.Hash{}, stateID-1)
			if err != nil {
				log.Info("DEBUG", "id", stateID, "accountHash", account.Hex(), "err", err)
			} else {
				if !bytes.Equal(data, exp) {
					log.Info("DEBUG", "id", stateID, "accountHash", account.Hex(), "data", data)
				}
			}
		}
	}
