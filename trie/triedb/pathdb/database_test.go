...
func updateTrie(addrHash common.Hash, root common.Hash, dirties, cleans map[common.Hash][]byte) (common.Hash, *trienode.NodeSet) {
	h, err := testutil.NewHasher(addrHash, root, cleans)
	if err != nil {
...
func (t *tester) generate(parent common.Hash) (common.Hash, *trienode.MergedNodeSet, *triestate.Set) {
	...
	t.snapAccounts[parent] = testutil.CopyAccounts(t.accounts)
	t.snapStorages[parent] = testutil.CopyStorages(t.storages)
	...
	return root, ctx.nodes, triestate.New(parent, root, ctx.accountOrigin, ctx.storageOrigin)
}
...
func (t *tester) verifyState(root common.Hash) error {
	reader := t.db.Reader(root)
	if reader == nil {
		return errors.New("state is not available")
	}
...
func (t *tester) verifyHistory() error {
	...
	for i, root := range t.roots {
		...
		if i > bottom {
			_, err := loadHistory(t.db.freezer, uint64(i+1))
			if err == nil {
		...
		// should exist.
		obj, err := loadHistory(t.db.freezer, uint64(i+1))
		if err != nil {
			return err
		}
...
func TestDatabaseRollback(t *testing.T) {
	...
	for i := tester.bottomIndex(); i >= 0; i-- {
		...
		loader := testutil.NewHashLoader(tester.snapAccounts[root], tester.snapStorages[root])
...
func TestDatabaseRecoverable(t *testing.T) {
	...
	for _, c := range cases {
		...
		if result != c.expect {
			t.Fatalf("Unexpected result, want %t, got %t", c.expect, result)
		}
	}
}
...
func TestReset(t *testing.T) {
	...
	for i := 0; i <= index; i++ {
		_, err := loadHistory(tester.db.freezer, uint64(i+1))
		if err == nil {
