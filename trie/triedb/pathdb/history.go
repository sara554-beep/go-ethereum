...
func (i *accountIndex) encode() []byte {
	...
	binary.BigEndian.PutUint32(buf[common.HashLength+9:], i.storageSlots)
	return buf[:]
}
...
func (i *accountIndex) decode(blob []byte) {
	...
	i.storageSlots = binary.BigEndian.Uint32(blob[common.HashLength+9:])
}
...
func (m *meta) encode() []byte {
	buf := make([]byte, historyMetaSize+len(m.incomplete)*common.HashLength)
	buf[0] = m.version
	...
	for i, h := range m.incomplete {
		copy(buf[i*common.HashLength+historyMetaSize:], h.Bytes())
	}
	return buf[:]
}
...
func (m *meta) decode(blob []byte) error {
	...
	case stateHistoryVersion:
		...
		if (len(blob)-historyMetaSize)%common.HashLength != 0 {
...
func (h *history) decode(accountData, storageData, accountIndexes, storageIndexes []byte) error {
	...
	for i := 0; i < len(accountIndexes)/accountIndexSize; i++ {
		...
		accounts[accIndex.hash] = accData
		accountList = append(accountList, accIndex.hash)
		...
		if len(slotList) > 0 {
			storageList[accIndex.hash] = slotList
			storages[accIndex.hash] = slotData
		}
	}
	h.accounts = accounts
