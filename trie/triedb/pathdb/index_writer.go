...
// indexBlockDesc is the descriptor of an index block which contains a list of
// state mutation records belong to the specific state(account or storage slot).
...
func (w *indexWriter) finish(batch ethdb.Batch) error {
	...
	for _, bw := range writers {
		...
		rawdb.WriteStateIndexBlock(batch, w.owner, w.state, bw.desc.id, bw.buf)
	}
	buf := make([]byte, 0, indexBlockDescSize*len(descList))
	for _, desc := range descList {
		buf = append(buf, desc.encode()...)
	}
	rawdb.WriteStateIndex(batch, w.owner, w.state, buf)
	return nil
}
...
func (w *writer) reset() {
	w.states = 0
	w.accounts = make(map[common.Hash][]uint64)
	w.storages = make(map[common.Hash]map[common.Hash][]uint64)
}

func (w *writer) addAccount(hash common.Hash, number uint64) {
	w.states += 1
	w.accounts[hash] = append(w.accounts[hash], number)
}

func (w *writer) addSlot(addrHash common.Hash, hash common.Hash, number uint64) {
	w.states += 1
	if _, ok := w.storages[addrHash]; !ok {
		w.storages[addrHash] = make(map[common.Hash][]uint64)
	}
...
func (w *writer) finish(db ethdb.KeyValueStore, force bool, head uint64) error {
	...
	for account, idList := range w.accounts {
		iw, err := newIndexWriter(db, common.Hash{}, account)
		if err != nil {
			return err
		}
	...
	rawdb.WriteStateIndexHead(batch, head)
