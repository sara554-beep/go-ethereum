...
func stackTrieFromPool(writer func(common.Hash, []byte, []byte), owner common.Hash) *StackTrie {
...
type StackTrie struct {
	...
	writer   func(common.Hash, []byte, []byte) // Function for flushing nodes, can be nil
}

// NewStackTrie allocates and initializes an empty trie.
func NewStackTrie(writer func(common.Hash, []byte, []byte)) *StackTrie {
	return &StackTrie{
		nodeType: emptyNode,
		writer:   writer,
	}
...
func NewStackTrieWithOwner(writer func(common.Hash, []byte, []byte), owner common.Hash) *StackTrie {
	return &StackTrie{
		owner:    owner,
		nodeType: emptyNode,
		writer:   writer,
...
func NewFromBinary(data []byte, writer func(common.Hash, []byte, []byte)) (*StackTrie, error) {
	var st StackTrie
...
func (st *StackTrie) setDb(writer func(common.Hash, []byte, []byte)) {
	st.writer = writer
	for _, child := range st.children {
		if child != nil {
			child.setDb(writer)
...
func newLeaf(owner common.Hash, key, val []byte, writer func(common.Hash, []byte, []byte)) *StackTrie {
...
func newExt(owner common.Hash, key []byte, child *StackTrie, writer func(common.Hash, []byte, []byte)) *StackTrie {
	st := stackTrieFromPool(writer, owner)
	st.nodeType = extNode
	st.key = append(st.key, key...)
	st.children[0] = child
...
func (st *StackTrie) hashRec(hasher *hasher, path []byte) {
	...
	case extNode:
		...
		st.children[0].hashRec(hasher, prefix)
	...
	case leafNode:
		st.key = append(append([]byte{}, st.key...), byte(16))
		sz := hexToCompactInPlace(st.key)
	...
	if st.writer != nil {
		...
		st.writer(common.BytesToHash(st.val), key, encodedNode)
	}
}
