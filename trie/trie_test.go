...
func TestMissingRootHashBased(t *testing.T) { testMissingRoot(t, rawdb.HashScheme) }
func TestMissingRootPathBased(t *testing.T) { testMissingRoot(t, rawdb.PathScheme) }
...
func TestReplication(t *testing.T) {
	...
	if newhash := trie2.Hash(); newhash != hash {
		t.Errorf("root failure. expected %x got %x", hash, newhash)
	}
}
...
func benchGet(b *testing.B) {
	triedb := NewHashDatabase(rawdb.NewMemoryDatabase())
	trie := NewEmpty(triedb)
	k := make([]byte, 32)
...
func TestTinyTrie(t *testing.T) {
	...
	checktr := NewEmpty(NewHashDatabase(rawdb.NewMemoryDatabase()))
...
func TestCommitSequence(t *testing.T) {
	for i, tc := range []struct {
		...
		db := NewHashDatabase(rawdb.NewDatabase(s))
		trie := NewEmpty(db)
