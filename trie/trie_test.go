...
func testMissingNode(t *testing.T, memonly bool, scheme string) {
	...
	trie, _ = New(root, common.Hash{}, root, triedb)
	if memonly {
		trie.nodes.banned = map[string]struct{}{string(key): {}}
	} else {
		triedb.Scheme().DeleteTrieNode(diskdb, key, hash)
	}
	_, err = trie.TryGet([]byte("120000"))
	if _, ok := err.(*MissingNodeError); !ok {
...
func runRandTest(rt randTest) bool {
	...
	for i, step := range rt {
		...
		case opCommit:
			...
			if root != original {
				err = triedb.Update(root, original, nodes)
				if err != nil {
					rt[i].err = err
					return false
				}
