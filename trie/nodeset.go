...
// Fix is a special function to fix incorrect prev value recorded in nodeset.
func (set *NodeSet) Fix(orig *StateTrie) error {
	for path, n := range set.nodes {
		blob, _, err := orig.TryGetNode([]byte(path))
		if err != nil {
			return err
		}
		n.prev = blob
