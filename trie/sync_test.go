...
func makeTestTrie(scheme string) (ethdb.Database, *triedb.Database, *StateTrie, map[string][]byte) {
...
func TestEmptySync(t *testing.T) {
	...
	for i, trie := range []*Trie{emptyA, emptyB, emptyC, emptyD} {
		sync := NewSync(trie.Hash(), memorydb.New(), nil, []*triedb.Database{dbA, dbB, dbC, dbD}[i].Scheme())
...
func syncWith(t *testing.T, root common.Hash, db ethdb.Database, srcDb *triedb.Database) {
	syncWithHookWriter(t, root, db, srcDb, nil)
}

func syncWithHookWriter(t *testing.T, root common.Hash, db ethdb.Database, srcDb *triedb.Database, hookWriter ethdb.KeyValueWriter) {
	// Create a destination trie and sync with the scheduler
