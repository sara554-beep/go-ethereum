	...
	for key, obj := range db.dirties {
		if obj != nil && key != "" {
			if _, ok := elements[key]; !ok {
				t.Errorf("state entry not reported %v", []byte(key))
			}
		}
	}
...
func testIteratorContinueAfterError(t *testing.T, memonly bool) {
	...
	for i := 0; i < 20; i++ {
		...
		if memonly {
			...
			rawdb.DeleteTrieNode3(diskdb, rkey, rhash)
		}
		...
		if memonly {
			triedb.dirties[string(rkey)] = robj
		} else {
...
func testIteratorContinueAfterSeekError(t *testing.T, memonly bool) {
	...
	if !memonly {
		triedb.Commit(root, true, func(key, _ []byte) {
			_, hash := rawKey(key)
			if hash == barNodeHash {
	...
	if memonly {
		...
		rawdb.DeleteTrieNode3(diskdb, rawkey, hash)
		barNodeBlob = blob
	}
