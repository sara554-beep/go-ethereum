...
func TestNodeIteratorCoverage(t *testing.T) {
	...
	for it.Next() {
		...
		if elem, ok := elements[string(nodeKey)]; !ok {
...
func TestIteratorNoDups(t *testing.T) {
	tr, _ := New(common.Hash{}, common.Hash{}, common.Hash{}, NewDatabase(rawdb.NewMemoryDatabase(), nil))
	for _, val := range testdata1 {
...
func testIteratorContinueAfterError(t *testing.T, memonly bool) {
	...
	tr, _ := New(common.Hash{}, common.Hash{}, common.Hash{}, tdb)
	for _, val := range testdata1 {
		tr.Update([]byte(val.k), []byte(val.v))
	}
	root, nodes, _ := tr.Commit(nil)
	tdb.Commit(root, common.Hash{}, nodes)
	if !memonly {
		tdb.Cap(root, 0)
	}
	...
	for i := 0; i < 20; i++ {
		...
		if memonly {
			tr.nodes.nodes[string(rkey)] = &cachedNode{hash: rhash, node: simplifyNode(rnode), size: 100}
		} else {
...
func testIteratorContinueAfterSeekError(t *testing.T, memonly bool) {
	...
	ctr, _ := New(common.Hash{}, common.Hash{}, common.Hash{}, triedb)
	for _, val := range testdata1 {
		ctr.Update([]byte(val.k), []byte(val.v))
	}
	root, nodes, _ := ctr.Commit(nil)
	...
	if memonly {
		...
		n, exist := snap.nodes[string(barNodeKey)]
		if !exist {
		...
		barNodeObj = n
		delete(snap.nodes, string(barNodeKey))
	} else {
	...
	if memonly {
		snap := triedb.GetReader(root).(*diffLayer)
		snap.nodes[string(barNodeKey)] = barNodeObj
	} else {
...
func TestIteratorNodeBlob(t *testing.T) {
	...
	var count int
	for dbIter.Next() {
		isnode, key := rawdb.IsTrieNodeKey(dbIter.Key())
		if !isnode {
			continue
		}
		got, present := found[string(key)]
		if !present {
