...
func fill(db *Database, n int, testkeys [][]string, testvals [][][]byte, parentHash common.Hash, parentBlob []byte, parentNumber uint64) (common.Hash, []byte, uint64, []string, [][]byte) {
	...
	for i := 0; i < n; i++ {
		...
		if node.node == nil {
			...
			dels = append(dels, storage)
		} else {
			vals = append(vals, common.CopyBytes(node.rlp()))
			inserts = append(inserts, storage)
		}
	}
	// Add the root node
	root := randomNode()
	...
	db.Update(root.hash, parentHash, &NodeSet{nodes: nodes, keys: inserts, dels: dels})
	return root.hash, root.rlp(), parentNumber + 1, keys, vals
}

func fillDB(t *testing.T, scheme string) *testEnv {
	var (
		diskdb, _ = rawdb.NewDatabaseWithFreezer(rawdb.NewMemoryDatabase(), t.TempDir(), "", false)
		db        = NewDatabase(diskdb, &Config{Scheme: scheme})
		numbers   []uint64
		roots     []common.Hash
		testKeys  [][]string
		testVals  [][][]byte
...
func TestCommit(t *testing.T) {
	...
	if err := snapdb.Commit(env.roots[len(env.roots)-1]); err != nil {
