...
func fillDB(t *testing.T, scheme string) *testEnv {
	...
	for i := 0; i < 2*128; i++ {
		...
		db.Update(root, parent, NewWithNodeSet(set))
		parent = root
	}
	return &testEnv{
		db:    db,
...
func TestCommit(t *testing.T) {
	...
	if err := db.Commit(env.roots[len(env.roots)-1]); err != nil {
