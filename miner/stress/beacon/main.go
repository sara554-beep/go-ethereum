...
func (n *ethNode) assembleBlock(parentHash common.Hash, parentTimestamp uint64) (*interop.ExecutableDataV1, error) {
	...
	payloadAttribute := interop.PayloadAttributesV1{
		Timestamp:             timestamp,
		Random:                common.Hash{},
...
func (n *ethNode) insertBlock(eb interop.ExecutableDataV1) error {
	if !eth2types(n.typ) {
...
func (n *ethNode) insertBlockAndSetHead(parent *types.Header, ed interop.ExecutableDataV1) error {
	if !eth2types(n.typ) {
		return errors.New("invalid node type")
	}
