...
func TestDAOForkRangeExtradata(t *testing.T) {
	...
	SetupGenesisBlock(gendb, gspec)
	prefix, _ := GenerateChain(params.TestChainConfig, gspec.ToBlock(), ethash.NewFaker(), gendb, int(forkBlock.Int64()-1), func(i int, gen *BlockGen) {})
	...
	proBc, _ := NewBlockChain(proDb, proGspec, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer proBc.Stop()
	...
	conBc, _ := NewBlockChain(conDb, conGspec, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer conBc.Stop()
	...
	for i := int64(0); i < params.DAOForkExtraRange.Int64(); i++ {
		...
		bc, _ := NewBlockChain(gendb, nil, &conConf, ethash.NewFaker(), vm.Config{}, nil, nil)
		defer bc.Stop()
		...
		gendb = rawdb.NewMemoryDatabase()
		gspec.MustCommit(gendb)
		bc, _ = NewBlockChain(gendb, nil, &proConf, ethash.NewFaker(), vm.Config{}, nil, nil)
		defer bc.Stop()
