...
func (s *StateDB) deleteStorage(addrHash common.Hash, root common.Hash) (bool, map[common.Hash][]byte, *trienode.NodeSet, error) {
	...
	for it.Next(true) {
		...
		if stateSize+nodeSize > 512*opt.MiB {
			log.Info("Skip gigantic storage deletion", "addrHash", addrHash, "states", stateSize, "nodes", nodeSize)
			if metrics.EnabledExpensive {
...
func (s *StateDB) handleDestruction(nodes *trienode.MergedNodeSet) (map[common.Hash]struct{}, error) {
	...
	for addr, prev := range s.stateObjectsDestruct {
		...
		aborted, slots, set, err := s.deleteStorage(addrHash, prev.Root)
		if err != nil {
...
// Once the state is committed, it's not usable anymore and a new state
// instance must be created with new root upon the updated database for
// accessing latest state.
func (s *StateDB) Commit(deleteEmptyObjects bool) (common.Hash, error) {
	...
	if root != origin {
		start := time.Now()
		if err := s.db.TrieDB().Update(root, origin, nodes, triestate.New(s.accountsOrigin, s.storagesOrigin, incomplete)); err != nil {
			return common.Hash{}, err
		}
		s.originalRoot = root
		if metrics.EnabledExpensive {
