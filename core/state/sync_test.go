...
func checkTrieConsistency(db ethdb.Database, scheme trie.NodeScheme, root common.Hash) error {
	...
	trie, err := trie.New(trie.StateTrieID(root), trie.NewDatabase(db, &trie.Config{Scheme: scheme.Name()}))
	if err != nil {
		return err
	}
	...
	for len(nodeElements)+len(codeElements) > 0 {
		...
		for i, node := range nodeElements {
			if bypath {
				...
				data, err := srcDb.TrieDB().GetReader(srcRoot).NodeBlob(owner, inner, node.hash)
				if err != nil {
