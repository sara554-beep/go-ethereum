...
func checkTrieConsistency(db ethdb.Database, scheme trie.NodeScheme, root common.Hash) error {
	if !scheme.HasTrieNode(db, common.Hash{}, nil, root) {
	...
	for len(nodeElements)+len(codeElements) > 0 {
		...
		for i, node := range nodeElements {
			if bypath {
				...
				owner, inner := trie.ParsePath([]byte(node.path))
				data, err := srcDb.NodeDB().GetReader(srcRoot).NodeBlob(owner, inner, node.hash)
				if err != nil {
	...
	for len(nodeElements)+len(codeElements) > 0 {
		...
		if len(nodeElements) > 0 {
			...
			for i, element := range nodeElements[:len(nodeResults)] {
				owner, inner := trie.ParsePath([]byte(element.path))
				data, err := srcDb.NodeDB().GetReader(srcRoot).NodeBlob(owner, inner, element.hash)
				if err != nil {
					t.Fatalf("failed to retrieve contract bytecode for %x", element.code)
				}
...
func testIncompleteStateSync(t *testing.T, scheme string) {
	...
	for i, path := range addedNodes {
		owner, inner := trie.ParsePath([]byte(path))
		hash := addedHashes[i]
		val := ndb.Scheme().ReadTrieNode(dstDb, owner, inner, hash)
		if val == nil {
			t.Error("missing trie node")
		}
		ndb.Scheme().DeleteTrieNode(dstDb, owner, inner, hash)
		if err := checkStateConsistency(dstDb, ndb.Scheme(), srcRoot); err == nil {
			t.Errorf("trie inconsistency not caught, missing: %v", path)
		}
		ndb.Scheme().WriteTrieNode(dstDb, owner, inner, hash, val)
	}
}
