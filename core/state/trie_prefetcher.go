...
func (sf *subfetcher) openTrie() (Trie, error) {
	if sf.verkle {
		...
		return verkle.New(sf.state, sf.db, verkle.NewPointCache(commitmentCacheItems))
	}
	// Start by opening the trie and stop processing if it fails
	if sf.owner == (common.Hash{}) {
		return merkle.NewStateTrie(trie.StateTrieID(sf.state), sf.db)
	}
	return merkle.NewStateTrie(trie.StorageTrieID(sf.state, crypto.Keccak256Hash(sf.addr.Bytes()), sf.root), sf.db)
}
