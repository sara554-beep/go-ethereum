...
func newVerkleHasher(root common.Hash, db *triedb.Database, prefetcher *triePrefetcher) (*verkleHasher, error) {
	...
	if tr == nil {
		tr, err = verkle.New(root, db, verkle.NewPointCache(commitmentCacheItems))
		if err != nil {
			return nil, err
		}
	}
...
// verkle tree.
func newVerkleStorageHasher(trie *verkle.Trie, address common.Address, root common.Hash) *verkleStorageHasher {
