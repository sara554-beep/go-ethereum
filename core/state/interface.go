...
// AccountHasher wraps the methods of a backing account hasher, providing an
// interface for writing account changes back and recomputing the new state root.
type AccountHasher interface {
	// UpdateAccount abstracts an account write to the hasher. It encodes the
	...
	// Commit collects all mutations produced by hasher for state rehashing.
	// The nodeset can be nil if the state is not changed at all.
	Commit() (common.Hash, *trienode.NodeSet, error)

	// Copy returns a deep-copied account hasher.
	Copy() Hasher
}

// StorageHasher wraps the methods of a backing storage hasher, providing an
// interface for writing storage changes back and recomputing the new storage root.
type StorageHasher interface {
	// UpdateStorage abstracts a storage write to the hasher.
	UpdateStorage(address common.Address, key, value []byte) error

	// DeleteStorage abstracts a storage deletion from the hasher.
	...
	// Commit collects all mutations produced by hasher for storage rehashing.
...
type StorageDeleter interface {
	...
	// deleted slots along with associated proofs.
	Delete(address common.Address, root common.Hash) (map[common.Hash][]byte, *trienode.NodeSet, error)
}
