...
func TestIntermediateLeaks(t *testing.T) {
	...
	if err = transState.Database().TrieDB().(*trie.Database).Commit(transRoot); err != nil {
	...
	if err = finalState.Database().TrieDB().(*trie.Database).Commit(finalRoot); err != nil {
...
func TestMissingTrieNodes(t *testing.T) {
	...
	addr := common.BytesToAddress([]byte("so"))
	{
		...
		t.Logf("root: %x", root)
		// force-flush
		state.Database().TrieDB().(*trie.Database).Commit(root)
	}
