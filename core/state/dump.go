...
func (db *StateDB) DumpToCollector(c DumpCollector, conf *DumpConfig) (nextKey []byte) {
	...
	log.Info("Trie dumping started", "root", db.getTrie().Hash())
	c.OnRoot(db.getTrie().Hash())

	it := trie.NewIterator(db.getTrie().NodeIterator(conf.Start))
	for it.Next() {
		...
		addrBytes := db.getTrie().GetKey(it.Key)
		if addrBytes == nil {
			// Preimage missing
			missingPreimages++
		...
		if !conf.SkipStorage {
			...
			for storageIt.Next() {
				...
				account.Storage[common.BytesToHash(db.getTrie().GetKey(storageIt.Key))] = common.Bytes2Hex(content)
			}
		}
