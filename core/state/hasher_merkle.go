...
func newMerkleHasher(root common.Hash, db *trie.Database, prefetcher *triePrefetcher) (*merkleHasher, error) {
	var (
		tr  Trie
		err error
	)
...
func newMerkleStorageHasher(stateRoot common.Hash, address common.Address, root common.Hash, db *trie.Database, prefetcher *triePrefetcher) (*merkleStorageHasher, error) {
	var (
		tr       Trie
		err      error
	...
	if tr == nil {
		tr, err = trie.NewStateTrie(trie.StorageTrieID(stateRoot, addrHash, root), db)
		if err != nil {
			return nil, err
		}
	}
	return &merkleStorageHasher{trie: tr.(*trie.StateTrie)}, nil
}
