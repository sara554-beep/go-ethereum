...
// len returns the total cached layer in the tree.
...
func (tree *layerTree) add(root common.Hash, parentRoot common.Hash, destructs map[common.Hash]struct{}, accounts map[common.Hash][]byte, storage map[common.Hash]map[common.Hash][]byte) error {
	...
	snap := parent.Update(root, parent.ID()+1, destructs, accounts, storage)
...
func (tree *layerTree) cap(root common.Hash, layers int, freezer *rawdb.ResettableFreezer) error {
	...
	if layers == 0 {
		base, err := diff.persist(true, freezer)
		if err != nil {
			return err
		}
	...
	case *diffLayer:
		...
		base, err := parent.persist(false, freezer)
		if err != nil {
			diff.lock.Unlock()
			return err
		}
