...
func newNodeIterator(state *StateDB) (*nodeIterator, error) {
	...
	tr, err := merkle.NewStateTrie(trie.TrieID(state.originalRoot), state.db.TrieDB())
	if err != nil {
		return nil, err
	}
	return &nodeIterator{
...
func (it *nodeIterator) step() error {
	...
	dataTrie, err := merkle.NewStateTrie(trie.StorageTrieID(it.state.originalRoot, common.BytesToHash(it.stateIt.LeafKey()), account.Root), it.state.db.TrieDB())
	if err != nil {
		return err
	}
