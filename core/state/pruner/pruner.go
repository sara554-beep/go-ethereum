...
func extractGenesis(db ethdb.Database, stateBloom *stateBloom) error {
	...
	t, err := trie.NewSecure(genesis.Root(), common.Hash{}, genesis.Root(), trie.NewDatabase(db, &trie.Config{Scheme: trie.HashScheme}))
	if err != nil {
		return err
	}
	...
	for accIter.Next(true) {
		...
		if accIter.Leaf() {
			...
			if acc.Root != emptyRoot {
				storageTrie, err := trie.NewSecure(genesis.Root(), common.BytesToHash(accIter.LeafKey()), acc.Root, trie.NewDatabase(db, &trie.Config{Scheme: trie.HashScheme}))
				if err != nil {
					return err
				}
