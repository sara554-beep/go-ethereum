...
func NewPruner(db ethdb.Database, config Config) (*Pruner, error) {
	...
	snaptree, err := snapshot.New(snapconfig, db, trie.NewDatabase(db, nil), headBlock.Root())
	if err != nil {
...
func extractGenesis(db ethdb.Database, stateBloom *stateBloom) error {
	...
	t, err := trie.NewStateTrie(trie.StateTrieID(genesis.Root()), trie.NewDatabase(db, nil))
	if err != nil {
		return err
	}
	...
	for accIter.Next(true) {
		...
		if accIter.Leaf() {
			...
			if acc.Root != types.EmptyRootHash {
				...
				storageTrie, err := trie.NewStateTrie(id, trie.NewDatabase(db, nil))
				if err != nil {
					return err
				}
