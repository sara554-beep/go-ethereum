...
func testHeaderVerificationForMerging(t *testing.T, isClique bool) {
	...
	if isClique {
		...
		postBlocks, _ = GenerateChain(&config, preBlocks[len(preBlocks)-1], engine, gendb, 8, nil)
	} else {
		gspec = &Genesis{Config: params.TestChainConfig}
		...
		config.TerminalTotalDifficulty = big.NewInt(int64(td))
		postBlocks, _ = GenerateChain(params.TestChainConfig, preBlocks[len(preBlocks)-1], engine, gendb, 8, nil)
	}
