...
func testReorg(t *testing.T, first, second []int64, td int64, full bool) {
	...
	easyBlocks, _ := GenerateChainWithInMemoryDB(params.TestChainConfig, blockchain.CurrentBlock(), ethash.NewFaker(), db, len(first), func(i int, b *BlockGen) {
		b.OffsetTime(first[i])
	})
	diffBlocks, _ := GenerateChainWithInMemoryDB(params.TestChainConfig, blockchain.CurrentBlock(), ethash.NewFaker(), db, len(second), func(i int, b *BlockGen) {
...
func testReorgBadHashes(t *testing.T, full bool) {
	// Create a pristine chain and database
	statedb, blockchain, err := newCanonical(ethash.NewFaker(), 0, full)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}

	// Create a chain, import and ban afterwards
	headers := makeHeaderChain(blockchain.CurrentHeader(), 4, ethash.NewFaker(), statedb, 10)
	blocks := makeBlockChain(blockchain.CurrentBlock(), 4, ethash.NewFaker(), statedb, 10)
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	archiveCaching := defaultWithStateFreezer(t.TempDir())
	...
	fastDb := makeDb()
	defer fastDb.Close()
	fast, _ := NewBlockChain(fastDb, defaultWithStateFreezer(t.TempDir()), gspec.Config, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer fast.Stop()
	...
	ancientDb := makeDb()
	defer ancientDb.Close()
	ancient, _ := NewBlockChain(ancientDb, defaultWithStateFreezer(t.TempDir()), gspec.Config, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer ancient.Stop()
	...
	lightDb := makeDb()
	defer lightDb.Close()
	light, _ := NewBlockChain(lightDb, defaultWithStateFreezer(t.TempDir()), gspec.Config, ethash.NewFaker(), vm.Config{}, nil, nil)
...
func TestChainTxReorgs(t *testing.T) {
	...
	chain, _ := GenerateChainWithInMemoryDB(gspec.Config, genesis, ethash.NewFaker(), statedb, 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
	...
	chain, _ = GenerateChainWithInMemoryDB(gspec.Config, genesis, ethash.NewFaker(), statedb, 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
...
func TestLogReorgs(t *testing.T) {
	...
	chain, _ := GenerateChainWithInMemoryDB(params.TestChainConfig, genesis, ethash.NewFaker(), statedb, 2, func(i int, gen *BlockGen) {
		if i == 1 {
	...
	chain, _ = GenerateChainWithInMemoryDB(params.TestChainConfig, genesis, ethash.NewFaker(), statedb, 3, func(i int, gen *BlockGen) {})
...
func TestLogRebirth(t *testing.T) {
	...
	chain, _ := GenerateChainWithInMemoryDB(params.TestChainConfig, genesis, engine, statedb, 2, func(i int, gen *BlockGen) {
		if i == 1 {
	...
	forkChain, _ := GenerateChainWithInMemoryDB(params.TestChainConfig, genesis, engine, statedb, 2, func(i int, gen *BlockGen) {
		if i == 1 {
			tx, err := types.SignTx(types.NewContractCreation(gen.TxNonce(addr1), new(big.Int), 1000000, gen.header.BaseFee, logCode), signer, key1)
			if err != nil {
				t.Fatalf("failed to create tx: %v", err)
	...
	newBlocks, _ := GenerateChainWithInMemoryDB(params.TestChainConfig, chain[len(chain)-1], engine, statedb, 1, func(i int, gen *BlockGen) {})
...
func TestSideLogRebirth(t *testing.T) {
	...
	sideChain, _ := GenerateChainWithInMemoryDB(params.TestChainConfig, genesis, ethash.NewFaker(), statedb, 2, func(i int, gen *BlockGen) {
		if i == 1 {
			tx, err := types.SignTx(types.NewContractCreation(gen.TxNonce(addr1), new(big.Int), 1000000, gen.header.BaseFee, logCode), signer, key1)
			if err != nil {
				t.Fatalf("failed to create tx: %v", err)
	...
	newBlocks, _ := GenerateChainWithInMemoryDB(params.TestChainConfig, sideChain[len(sideChain)-1], ethash.NewFaker(), statedb, 1, func(i int, gen *BlockGen) {})
	if _, err := blockchain.InsertChain(newBlocks); err != nil {
		t.Fatalf("failed to insert forked chain: %v", err)
	}
	checkLogEvents(t, newLogCh, rmLogsCh, 1, 0)
...
func TestReorgSideEvent(t *testing.T) {
	...
	replacementBlocks, _ := GenerateChainWithInMemoryDB(gspec.Config, genesis, ethash.NewFaker(), statedb, 4, func(i int, gen *BlockGen) {
...
func TestBlockchainHeaderchainReorgConsistency(t *testing.T) {
	...
	genesis := (&Genesis{BaseFee: big.NewInt(params.InitialBaseFee)}).MustCommit(db)
	statedb := state.NewDatabase(db)
	blocks, _ := GenerateChainWithInMemoryDB(params.TestChainConfig, genesis, engine, statedb, 64, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	...
	for i := 0; i < len(forks); i++ {
		...
		fork, _ := GenerateChainWithInMemoryDB(params.TestChainConfig, parent, engine, statedb, 1, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{2}) })
		forks[i] = fork[0]
	}
...
func TestLargeReorgTrieGC(t *testing.T) {
	...
	copyDB(db, gendb2)
	competitor, _ := GenerateChain(params.TestChainConfig, shared[len(shared)-1], engine, gendb2, 2*TriesInMemory+1, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{3}) })

	// Import the shared chain and the original canonical one
	frdir := t.TempDir()
	diskdb, err := rawdb.NewDatabaseWithFreezer(rawdb.NewMemoryDatabase(), frdir, "", false)
	if err != nil {
		t.Fatalf("failed to create temp freezer db: %v", err)
	}
	(&Genesis{BaseFee: big.NewInt(params.InitialBaseFee)}).MustCommit(diskdb)
	...
	if snap := chain.stateCache.TrieDB().Snapshot(shared[len(shared)-1].Root()); snap != nil {
	...
	for i, block := range competitor[:len(competitor)-2] {
		if snap := chain.stateCache.TrieDB().Snapshot(block.Root()); snap != nil {
	...
	if snap := chain.stateCache.TrieDB().Snapshot(competitor[len(competitor)-1].Root()); snap == nil {
		t.Fatalf("failed to reorg")
	}
...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int, mergePoint int) {
	...
	lastPrunedIndex := TriesInMemory - 2
	breakIndex := lastPrunedIndex + blocksBetweenCommonAncestorAndPruneblock
	blocks, dbs := genChainSegments(params.TestChainConfig, 2*TriesInMemory, []int{breakIndex}, genesis, db, engine, func(i int, b *BlockGen) {
		tx, err := types.SignTx(types.NewTransaction(b.TxNonce(addr), common.Address{0xff}, big.NewInt(1000), params.TxGas, b.BaseFee(), nil), signer, key)
		if err != nil {
			panic(err)
		}
		b.AddTx(tx)
	...
	chain, err := NewBlockChain(diskdb, nil, &chainConfig, engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	...
	firstNonPrunedBlock := blocks[lastPrunedIndex+1]
...
func testInsertKnownChainData(t *testing.T, typ string) {
	...
	blocks, receipts := GenerateChainWithInMemoryDB(params.TestChainConfig, genesis, engine, statedb, 32, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })

	// A longer chain but total difficulty is lower.
	blocks2, receipts2 := GenerateChainWithInMemoryDB(params.TestChainConfig, blocks[len(blocks)-1], engine, statedb, 65, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	// A shorter chain but total difficulty is higher.
	blocks3, receipts3 := GenerateChainWithInMemoryDB(params.TestChainConfig, blocks[len(blocks)-1], engine, statedb, 64, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
...
func testInsertKnownChainDataWithMerging(t *testing.T, typ string, mergeHeight int) {
	...
	blocks, receipts := GenerateChainWithInMemoryDB(&chainConfig, genesis, engine, statedb, 32, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	...
	blocks2, receipts2 := GenerateChainWithInMemoryDB(&chainConfig, blocks[len(blocks)-1], engine, statedb, 65, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })

	blocks3, receipts3 := GenerateChainWithInMemoryDB(&chainConfig, blocks[len(blocks)-1], engine, statedb, 64, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
...
func TestSideImportPrunedBlocks(t *testing.T) {
	...
	last := len(blocks) - 1
	lastPrunedIndex := last - TriesInMemory - 1
	lastPrunedBlock := blocks[lastPrunedIndex]
	...
	firstNonPrunedBlock := blocks[last-TriesInMemory]
	// Verify firstNonPrunedBlock is not pruned
	if !chain.HasBlockAndState(firstNonPrunedBlock.Hash(), firstNonPrunedBlock.NumberU64()) {
		t.Errorf("Block %d pruned", firstNonPrunedBlock.NumberU64())
	}
