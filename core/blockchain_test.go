...
func newCanonical(engine consensus.Engine, n int, full bool) (ethdb.Database, *BlockChain, error) {
	...
	blockchain, _ := NewBlockChain(db, gspec, nil, params.AllEthashProtocolChanges, engine, vm.Config{}, nil, nil)
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	// Initialize a fresh chain with only a genesis block
	blockchain, _ := NewBlockChain(db, gspec, nil, params.AllEthashProtocolChanges, ethash.NewFaker(), vm.Config{}, nil, nil)
	...
	ncm, err := NewBlockChain(blockchain.db, gspec, nil, blockchain.chainConfig, ethash.NewFaker(), vm.Config{}, nil, nil)
	if err != nil {
...
func TestFastVsFullChains(t *testing.T) {
	...
	archive, _ := NewBlockChain(archiveDb, gspec, nil, gspec.Config, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, gspec, nil, gspec.Config, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, gspec, nil, gspec.Config, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer ancient.Stop()
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	archive, _ := NewBlockChain(archiveDb, gspec, &archiveCaching, gspec.Config, ethash.NewFaker(), vm.Config{}, nil, nil)
	if n, err := archive.InsertChain(blocks); err != nil {
		t.Fatalf("failed to process block %d: %v", n, err)
	}
	defer archive.Stop()
	...
	defer delfn()
	light, _ := NewBlockChain(lightDb, gspec, nil, gspec.Config, ethash.NewFaker(), vm.Config{}, nil, nil)
...
func TestChainTxReorgs(t *testing.T) {
	...
	blockchain, _ := NewBlockChain(db, gspec, nil, gspec.Config, ethash.NewFaker(), vm.Config{}, nil, nil)
...
func TestLogRebirth(t *testing.T) {
	var (
		...
		blockchain, _ = NewBlockChain(db, gspec, nil, gspec.Config, engine, vm.Config{}, nil, nil)
	)
	gspec.MustCommit(gendb)
	gspec.MustCommit(gendb2)
...
func TestInsertReceiptChainRollback(t *testing.T) {
	...
	gspec.MustCommit(ancientDb)
	ancientChain, _ := NewBlockChain(ancientDb, &gspec, nil, gspec.Config, ethash.NewFaker(), vm.Config{}, nil, nil)
...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int, mergePoint int) {
	...
	gspec.MustCommit(diskdb)
	chain, err := NewBlockChain(diskdb, gspec, nil, &chainConfig, engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
...
func testInsertKnownChainData(t *testing.T, typ string) {
	...
	chain, err := NewBlockChain(chaindb, gspec, nil, params.TestChainConfig, engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
...
func testInsertKnownChainDataWithMerging(t *testing.T, typ string, mergeHeight int) {
	...
	gendb := rawdb.NewMemoryDatabase()
	copyDB(db, gendb)
	blocks, receipts := GenerateChain(&chainConfig, genesis, engine, gendb, 32, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	...
	gendb2 := rawdb.NewMemoryDatabase()
	copyDB(gendb, gendb2)
	blocks2, receipts2 := GenerateChain(&chainConfig, blocks[len(blocks)-1], engine, gendb2, 65, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })

	gendb3 := rawdb.NewMemoryDatabase()
	copyDB(gendb, gendb3)
	blocks3, receipts3 := GenerateChain(&chainConfig, blocks[len(blocks)-1], engine, gendb3, 64, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
	...
	chain, err := NewBlockChain(chaindb, gspec, nil, &chainConfig, engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	var (
...
func TestTransactionIndices(t *testing.T) {
	...
	l := uint64(0)
	chain, err := NewBlockChain(ancientDb, gspec, nil, params.TestChainConfig, ethash.NewFaker(), vm.Config{}, nil, &l)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	headers := make([]*types.Header, len(blocks))
...
func TestDeleteRecreateSlots(t *testing.T) {
	...
	diskdb := rawdb.NewMemoryDatabase()
	gspec.MustCommit(diskdb)
	chain, err := NewBlockChain(diskdb, gspec, nil, params.TestChainConfig, engine, vm.Config{
		Debug:  true,
...
func TestEIP2718Transition(t *testing.T) {
	...
	chain, err := NewBlockChain(diskdb, gspec, nil, gspec.Config, engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	if n, err := chain.InsertChain(blocks); err != nil {
