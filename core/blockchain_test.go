...
// header only chain.
func newCanonical(engine consensus.Engine, n int, full bool, scheme string) (ethdb.Database, ethdb.Database, *BlockChain, error) {
	var (
	...
	blockchain, _ := NewBlockChain(db, DefaultCacheConfigWithScheme(scheme), params.AllEthashProtocolChanges, engine, vm.Config{}, nil, nil)
	// Create and inject the requested chain
	if n == 0 {
		return db, gendb, blockchain, nil
	}
	if full {
		// Full block-chain requested
		blocks := makeBlockChain(genesis, n, engine, gendb, canonicalSeed)
		_, err := blockchain.InsertChain(blocks)
		return db, gendb, blockchain, err
	}
	// Header-only chain requested
	headers := makeHeaderChain(genesis.Header(), n, engine, gendb, canonicalSeed)
...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, comparator func(td1, td2 *big.Int), scheme string) {
	...
	_, gendb, blockchain2, err := newCanonical(ethash.NewFaker(), i, full, scheme)
	if err != nil {
	...
	if full {
		blockChainB = makeBlockChain(blockchain2.CurrentBlock(), n, ethash.NewFaker(), gendb, forkSeed)
		if _, err := blockchain2.InsertChain(blockChainB); err != nil {
			t.Fatalf("failed to insert forking chain: %v", err)
		}
	} else {
		headerChainB = makeHeaderChain(blockchain2.CurrentHeader(), n, ethash.NewFaker(), gendb, forkSeed)
...
func testLastBlock(t *testing.T, scheme string) {
	_, gendb, blockchain, err := newCanonical(ethash.NewFaker(), 0, true, scheme)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()

	blocks := makeBlockChain(blockchain.CurrentBlock(), 1, ethash.NewFullFaker(), gendb, 0)
...
func testBrokenChain(t *testing.T, full bool, scheme string) {
	...
	_, gendb, blockchain, err := newCanonical(ethash.NewFaker(), 10, full, scheme)
	if err != nil {
		t.Fatalf("failed to make new canonical chain: %v", err)
	}
	defer blockchain.Stop()
	...
	if full {
		chain := makeBlockChain(blockchain.CurrentBlock(), 5, ethash.NewFaker(), gendb, forkSeed)[1:]
		if err := testBlockChainImport(chain, blockchain); err == nil {
			t.Errorf("broken block chain not reported")
		}
	} else {
		chain := makeHeaderChain(blockchain.CurrentHeader(), 5, ethash.NewFaker(), gendb, forkSeed)[1:]
...
func testReorg(t *testing.T, first, second []int64, td int64, full bool, scheme string) {
	...
	_, gendb, blockchain, err := newCanonical(ethash.NewFaker(), 0, full, scheme)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()
...
func testBadHashes(t *testing.T, full bool, scheme string) {
	...
	if full {
		blocks := makeBlockChain(blockchain.CurrentBlock(), 3, ethash.NewFaker(), gendb, 10)
		...
		headers := makeHeaderChain(blockchain.CurrentHeader(), 3, ethash.NewFaker(), gendb, 10)
...
func testReorgBadHashes(t *testing.T, full bool, scheme string) {
	...
	blocks := makeBlockChain(blockchain.CurrentBlock(), 4, ethash.NewFaker(), gendb, 10)
	...
	ncm, err := NewBlockChain(blockchain.db, DefaultCacheConfigWithScheme(scheme), blockchain.chainConfig, ethash.NewFaker(), vm.Config{}, nil, nil)
	if err != nil {
...
func testInsertNonceError(t *testing.T, full bool, scheme string) {
	for i := 1; i < 25 && !t.Failed(); i++ {
		...
		if full {
			blocks := makeBlockChain(blockchain.CurrentBlock(), i, ethash.NewFaker(), gendb, 0)
			...
			headers := makeHeaderChain(blockchain.CurrentHeader(), i, ethash.NewFaker(), gendb, 0)
...
func testFastVsFullChains(t *testing.T, scheme string) {
	...
	gspec.Commit(archiveDb, scheme)
	archive, _ := NewBlockChain(archiveDb, DefaultCacheConfigWithScheme(scheme), gspec.Config, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer archive.Stop()
	...
	gspec.Commit(fastDb, scheme)
	fast, _ := NewBlockChain(fastDb, DefaultCacheConfigWithScheme(scheme), gspec.Config, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer fast.Stop()
	...
	gspec.Commit(ancientDb, scheme)
	ancient, _ := NewBlockChain(ancientDb, DefaultCacheConfigWithScheme(scheme), gspec.Config, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer ancient.Stop()
...
func testLightVsFastVsFullChainHeads(t *testing.T, scheme string) {
	...
	lightDb := makeDb()
	defer lightDb.Close()
	light, _ := NewBlockChain(lightDb, DefaultCacheConfigWithScheme(scheme), gspec.Config, ethash.NewFaker(), vm.Config{}, nil, nil)
...
func testChainTxReorgs(t *testing.T, scheme string) {
	...
	gspec.Commit(db, scheme)
	blockchain, _ := NewBlockChain(db, DefaultCacheConfigWithScheme(scheme), gspec.Config, ethash.NewFaker(), vm.Config{}, nil, nil)
...
func testLogReorgs(t *testing.T, scheme string) {
	...
	chain, _ = GenerateChain(params.TestChainConfig, genesis, ethash.NewFaker(), gendb, 3, func(i int, gen *BlockGen) {})
...
func testLogRebirth(t *testing.T, scheme string) {
	...
	gspec.Commit(db, scheme)
	blockchain, _ := NewBlockChain(db, DefaultCacheConfigWithScheme(scheme), gspec.Config, engine, vm.Config{}, nil, nil)
	defer blockchain.Stop()
	...
	forkChain, _ := GenerateChain(params.TestChainConfig, genesis, engine, gendb, 2, func(i int, gen *BlockGen) {
		if i == 1 {
			tx, err := types.SignTx(types.NewContractCreation(gen.TxNonce(addr1), new(big.Int), 1000000, gen.header.BaseFee, logCode), signer, key1)
			if err != nil {
				t.Fatalf("failed to create tx: %v", err)
	...
	newBlocks, _ := GenerateChain(params.TestChainConfig, chain[len(chain)-1], engine, gendb, 1, func(i int, gen *BlockGen) {})
...
func testSideLogRebirth(t *testing.T, scheme string) {
	...
	newBlocks, _ := GenerateChain(params.TestChainConfig, sideChain[len(sideChain)-1], ethash.NewFaker(), gendb, 1, func(i int, gen *BlockGen) {})
	if _, err := blockchain.InsertChain(newBlocks); err != nil {
		t.Fatalf("failed to insert forked chain: %v", err)
	}
	checkLogEvents(t, newLogCh, rmLogsCh, 1, 0)
...
func testReorgSideEvent(t *testing.T, scheme string) {
	...
	replacementBlocks, _ := GenerateChain(gspec.Config, genesis, ethash.NewFaker(), gendb, 4, func(i int, gen *BlockGen) {
...
func testCanonicalBlockRetrieval(t *testing.T, scheme string) {
	...
	chain, _ := GenerateChain(blockchain.chainConfig, blockchain.genesisBlock, ethash.NewFaker(), gendb, 10, func(i int, gen *BlockGen) {})
...
func testEIP155Transition(t *testing.T, scheme string) {
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, ethash.NewFaker(), gendb, 4, func(i int, block *BlockGen) {
		var (
...
func testEIP161AccountRemoval(t *testing.T, scheme string) {
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, ethash.NewFaker(), gendb, 3, func(i int, block *BlockGen) {
		var (
			tx     *types.Transaction
			err    error
			signer = types.LatestSigner(gspec.Config)
...
func testBlockchainHeaderchainReorgConsistency(t *testing.T, scheme string) {
	...
	gendb := rawdb.NewMemoryDatabase()
	genesis := (&Genesis{BaseFee: big.NewInt(params.InitialBaseFee)}).MustCommit(gendb)
	blocks, _ := GenerateChain(params.TestChainConfig, genesis, engine, gendb, 64, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	...
	for i := 0; i < len(forks); i++ {
		...
		fork, _ := GenerateChain(params.TestChainConfig, parent, engine, gendb, 1, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{2}) })
		forks[i] = fork[0]
	}
	// Import the canonical and fork chain side by side, verifying the current block
	// and current header consistency
	diskdb := rawdb.NewMemoryDatabase()
	(&Genesis{BaseFee: big.NewInt(params.InitialBaseFee)}).Commit(diskdb, scheme)

	chain, err := NewBlockChain(diskdb, DefaultCacheConfigWithScheme(scheme), params.TestChainConfig, engine, vm.Config{}, nil, nil)
	if err != nil {
...
func testLargeReorgTrieGC(t *testing.T, scheme string) {
	...
	shared, _ := GenerateChain(params.TestChainConfig, genesis, engine, gendb, 64, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	original, _ := GenerateChain(params.TestChainConfig, shared[len(shared)-1], engine, gendb, 2*TriesInMemory, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{2}) })
	competitor, _ := GenerateChain(params.TestChainConfig, shared[len(shared)-1], engine, gendb, 2*TriesInMemory+1, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{3}) })
...
func testLowDiffLongChain(t *testing.T, scheme string) {
	...
	blocks, _ := GenerateChain(params.TestChainConfig, genesis, engine, gendb, 6*TriesInMemory, func(i int, b *BlockGen) {
	...
	parent := blocks[10]
	fork, _ := GenerateChain(params.TestChainConfig, parent, engine, gendb, 8*TriesInMemory, func(i int, b *BlockGen) {
...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int, mergePoint int) {
	...
	var (
		...
		signer     = types.LatestSigner(gspec.Config)
		genesis, _ = gspec.Commit(db, trie.HashScheme)
	)
...
func testInsertKnownChainData(t *testing.T, typ string, scheme string) {
	...
	blocks, receipts := GenerateChain(params.TestChainConfig, genesis, engine, gendb, 32, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	// A longer chain but total difficulty is lower.
	blocks2, receipts2 := GenerateChain(params.TestChainConfig, blocks[len(blocks)-1], engine, gendb, 65, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	// A shorter chain but total difficulty is higher.
	blocks3, receipts3 := GenerateChain(params.TestChainConfig, blocks[len(blocks)-1], engine, gendb, 64, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
	...
	(&Genesis{BaseFee: big.NewInt(params.InitialBaseFee)}).Commit(chaindb, scheme)
	defer chaindb.Close()

	chain, err := NewBlockChain(chaindb, DefaultCacheConfigWithScheme(scheme), params.TestChainConfig, engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
...
func getLongAndShortChains(scheme string) (bc *BlockChain, longChain []*types.Block, heavyChain []*types.Block, err error) {
	...
	longChain, _ = GenerateChain(params.TestChainConfig, genesis, engine, gendb, 80, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
	})
	...
	heavyChainExt, _ := GenerateChain(params.TestChainConfig, parent, engine, gendb, 75, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{2})
		b.OffsetTime(-9)
	})
...
func testTransactionIndices(t *testing.T, scheme string) {
	...
	l := uint64(0)
	chain, err := NewBlockChain(ancientDb, DefaultCacheConfigWithScheme(scheme), params.TestChainConfig, ethash.NewFaker(), vm.Config{}, nil, &l)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	headers := make([]*types.Header, len(blocks))
...
func benchmarkLargeNumberOfValueToNonexisting(b *testing.B, numTxs, numBlocks int, recipientFn func(uint64) common.Address, dataFn func(uint64) []byte) {
	...
	shared, _ := GenerateChain(params.TestChainConfig, genesis, engine, gendb, numBlocks, blockGenerator)
	b.StopTimer()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		// Import the shared chain and the original canonical one
...
func testSideImportPrunedBlocks(t *testing.T, scheme string) {
	...
	// Generate and import the canonical chain
	blocks, _ := GenerateChain(params.TestChainConfig, genesis, engine, gendb, 2*TriesInMemory, nil)
	diskdb := rawdb.NewMemoryDatabase()
...
func testDeleteCreateRevert(t *testing.T, scheme string) {
	...
	blocks, _ := GenerateChain(params.TestChainConfig, genesis, engine, gendb, 1, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
...
func testDeleteRecreateSlots(t *testing.T, scheme string) {
	...
	diskdb := rawdb.NewMemoryDatabase()
	gspec.Commit(diskdb, scheme)
	chain, err := NewBlockChain(diskdb, DefaultCacheConfigWithScheme(scheme), params.TestChainConfig, engine, vm.Config{
		Debug:  true,
...
func testDeleteRecreateSlotsAcrossManyBlocks(t *testing.T, scheme string) {
	...
	blocks, _ := GenerateChain(params.TestChainConfig, genesis, engine, gendb, 150, func(i int, b *BlockGen) {
...
func testInitThenFailCreateContract(t *testing.T, scheme string) {
	...
	genesis := gspec.MustCommit(gendb)
	nonce := uint64(0)
	blocks, _ := GenerateChain(params.TestChainConfig, genesis, engine, gendb, 4, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
...
func testEIP2718Transition(t *testing.T, scheme string) {
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, engine, gendb, 1, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
	...
	chain, err := NewBlockChain(diskdb, DefaultCacheConfigWithScheme(scheme), gspec.Config, engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	if n, err := chain.InsertChain(blocks); err != nil {
...
func testSetCanonical(t *testing.T, scheme string) {
	...
	// Generate and import the canonical chain
	canon, _ := GenerateChain(params.TestChainConfig, genesis, engine, gendb, 2*TriesInMemory, func(i int, gen *BlockGen) {
	...
	side, _ := GenerateChain(params.TestChainConfig, genesis, engine, gendb, 2*TriesInMemory, func(i int, gen *BlockGen) {
...
func testCanonicalHashMarker(t *testing.T, scheme string) {
	...
	for _, c := range cases {
		...
		forkA, _ := GenerateChain(params.TestChainConfig, genesis, engine, gendb, c.forkA, func(i int, gen *BlockGen) {})
		forkB, _ := GenerateChain(params.TestChainConfig, genesis, engine, gendb, c.forkB, func(i int, gen *BlockGen) {})
