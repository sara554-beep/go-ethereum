...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, comparator func(td1, td2 *big.Int)) {
	...
	db, _, blockchain2, err := newCanonical(ethash.NewFaker(), i, full)
	if err != nil {
	...
	if full {
		blockChainB = makeBlockChain(blockchain2.chainConfig, blockchain2.CurrentBlock(), n, ethash.NewFaker(), db, forkSeed)
		if _, err := blockchain2.InsertChain(blockChainB); err != nil {
			t.Fatalf("failed to insert forking chain: %v", err)
		}
	} else {
		headerChainB = makeHeaderChain(blockchain2.chainConfig, blockchain2.CurrentHeader(), n, ethash.NewFaker(), db, forkSeed)
...
func TestLastBlock(t *testing.T) {
	...
	blocks := makeBlockChain(blockchain.chainConfig, blockchain.CurrentBlock(), 1, ethash.NewFullFaker(), blockchain.db, 0)
...
func testBrokenChain(t *testing.T, full bool) {
	...
	db, _, blockchain, err := newCanonical(ethash.NewFaker(), 10, full)
	if err != nil {
		t.Fatalf("failed to make new canonical chain: %v", err)
	}
	defer blockchain.Stop()
	...
	if full {
		chain := makeBlockChain(blockchain.chainConfig, blockchain.CurrentBlock(), 5, ethash.NewFaker(), db, forkSeed)[1:]
		if err := testBlockChainImport(chain, blockchain); err == nil {
			t.Errorf("broken block chain not reported")
		}
	} else {
		chain := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), 5, ethash.NewFaker(), db, forkSeed)[1:]
...
func testReorg(t *testing.T, first, second []int64, td int64, full bool) {
	...
	db, _, blockchain, err := newCanonical(ethash.NewFaker(), 0, full)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()
...
func testBadHashes(t *testing.T, full bool) {
	...
	if full {
		blocks := makeBlockChain(blockchain.chainConfig, blockchain.CurrentBlock(), 3, ethash.NewFaker(), db, 10)
		...
		headers := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), 3, ethash.NewFaker(), db, 10)
...
func testReorgBadHashes(t *testing.T, full bool) {
	// Create a pristine chain and database
	db, gspec, blockchain, err := newCanonical(ethash.NewFaker(), 0, full)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	// Create a chain, import and ban afterwards
	headers := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), 4, ethash.NewFaker(), db, 10)
	blocks := makeBlockChain(blockchain.chainConfig, blockchain.CurrentBlock(), 4, ethash.NewFaker(), db, 10)
...
func testInsertNonceError(t *testing.T, full bool) {
	for i := 1; i < 25 && !t.Failed(); i++ {
		...
		if full {
			blocks := makeBlockChain(blockchain.chainConfig, blockchain.CurrentBlock(), i, ethash.NewFaker(), db, 0)
			...
			headers := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), i, ethash.NewFaker(), db, 0)
...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int, mergePoint int) {
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, nil, runEngine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	...
	gendb, blocks, _ := GenerateChainWithGenesis(gspec, genEngine, 2*TriesInMemory, func(i int, gen *BlockGen) {
	...
	fork, _ := GenerateChain(gspec.Config, parent, genEngine, gendb, 2*TriesInMemory, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{2})
	})
...
func testInsertKnownChainData(t *testing.T, typ string) {
	...
	blocks, receipts := GenerateChain(params.TestChainConfig, genesis.ToBlock(), engine, gendb, 32, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
...
func testInsertKnownChainDataWithMerging(t *testing.T, typ string, mergeHeight int) {
	...
	var (
		gendb     = rawdb.NewMemoryDatabase()
		genesis   = &Genesis{BaseFee: big.NewInt(params.InitialBaseFee), Config: &chainConfig}
		runMerger = consensus.NewMerger(gendb)
		runEngine = beacon.New(ethash.NewFaker())
		genEngine = beacon.New(ethash.NewFaker())
	)
	...
	blocks, receipts := GenerateChain(&chainConfig, genesis.ToBlock(), genEngine, gendb, 32, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	...
	blocks2, receipts2 := GenerateChain(&chainConfig, blocks[len(blocks)-1], genEngine, gendb, 65, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	blocks3, receipts3 := GenerateChain(&chainConfig, blocks[len(blocks)-1], genEngine, gendb, 64, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
	...
	chain, err := NewBlockChain(chaindb, nil, genesis, nil, runEngine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	var (
