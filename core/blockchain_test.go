...
func newCanonical(engine consensus.Engine, n int, full bool) (ethdb.Database, *BlockChain, error) {
	...
	blockchain, _ := NewBlockChain(db, nil, params.AllEthashProtocolChanges, engine, vm.Config{}, nil, 0)
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	ncm, err := NewBlockChain(blockchain.db, nil, blockchain.chainConfig, ethash.NewFaker(), vm.Config{}, nil, 0)
	if err != nil {
...
func TestFastVsFullChains(t *testing.T) {
	...
	archive, _ := NewBlockChain(archiveDb, nil, gspec.Config, ethash.NewFaker(), vm.Config{}, nil, 0)
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, nil, gspec.Config, ethash.NewFaker(), vm.Config{}, nil, 0)
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, nil, gspec.Config, ethash.NewFaker(), vm.Config{}, nil, 0)
	defer ancient.Stop()
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	defer delfn()
	light, _ := NewBlockChain(lightDb, nil, gspec.Config, ethash.NewFaker(), vm.Config{}, nil, 0)
...
func TestChainTxReorgs(t *testing.T) {
	...
	blockchain, _ := NewBlockChain(db, nil, gspec.Config, ethash.NewFaker(), vm.Config{}, nil, 0)
...
func TestBlockchainHeaderchainReorgConsistency(t *testing.T) {
	...
	chain, err := NewBlockChain(diskdb, nil, params.TestChainConfig, engine, vm.Config{}, nil, 0)
	if err != nil {
...
func testInsertKnownChainData(t *testing.T, typ string) {
	...
	chain, err := NewBlockChain(chaindb, nil, params.TestChainConfig, engine, vm.Config{}, nil, 0)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
...
func TestTransactionIndices(t *testing.T) {
	...
	chain.InsertChain(blocks2[0:1])   // Feed chain a higher block to trigger indices updater.
	time.Sleep(50 * time.Millisecond) // Wait for indices initialisation
	tail := chain.CurrentBlock().NumberU64() - 64
	check(&tail, chain)
	chain.Stop()

	// Reconstruct a block chain which only reserves HEAD-32 tx indices, shorten the indices history.
	...
	time.Sleep(50 * time.Millisecond) // Wait for indices initialisation
	tail = chain.CurrentBlock().NumberU64() - 32
	check(&tail, chain)
	chain.Stop()

	// Reconstruct a block which reserves HEAD-128 tx indeces, extend the indices history.
	chain, err = NewBlockChain(ancientDb, nil, params.TestChainConfig, ethash.NewFaker(), vm.Config{}, nil, 128)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	chain.InsertChain(blocks2[2:3])   // Feed chain a higher block to trigger indices updater.
	time.Sleep(50 * time.Millisecond) // Wait for indices initialisation
	tail = chain.CurrentBlock().NumberU64() - 128
	check(&tail, chain)
	chain.Stop()
}
