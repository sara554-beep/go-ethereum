...
func newCanonical(engine consensus.Engine, n int, full bool) (ethdb.Database, *BlockChain, error) {
	...
	_, err := blockchain.InsertHeaderChain(headers, 1, false)
	return db, blockchain, err
}
...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, comparator func(td1, td2 *big.Int)) {
	...
	if full {
		...
		if _, err := blockchain2.InsertHeaderChain(headerChainB, 1, false); err != nil {
			t.Fatalf("failed to insert forking chain: %v", err)
		}
	}
...
func testReorg(t *testing.T, first, second []int64, td int64, full bool) {
	...
	if full {
		...
		if _, err := blockchain.InsertHeaderChain(easyHeaders, 1, false); err != nil {
			t.Fatalf("failed to insert easy chain: %v", err)
		}
		if _, err := blockchain.InsertHeaderChain(diffHeaders, 1, false); err != nil {
			t.Fatalf("failed to insert difficult chain: %v", err)
		}
	}
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	if full {
		...
		if _, err = blockchain.InsertHeaderChain(headers, 1, false); err != nil {
...
func testInsertNonceError(t *testing.T, full bool) {
	for i := 1; i < 25 && !t.Failed(); i++ {
		...
		if full {
			...
			failRes, err = blockchain.InsertHeaderChain(headers, 1, false)
		}
...
func TestFastVsFullChains(t *testing.T) {
	...
	if n, err := fast.InsertHeaderChain(headers, 1, false); err != nil {
		t.Fatalf("failed to insert header %d: %v", n, err)
	}
	if n, err := fast.InsertReceiptChain(blocks, receipts, false); err != nil {
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	if n, err := light.InsertHeaderChain(headers, 1, false); err != nil {
		t.Fatalf("failed to insert header %d: %v", n, err)
	}
	defer light.Stop()
