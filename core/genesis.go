...
func hashAlloc(ga *types.GenesisAlloc, isVerkle bool) (common.Hash, error) {
	...
	db := state.NewDatabase(state.NewCodeDB(memorydb), triedb.NewDatabase(memorydb, &config), nil)
	...
	return statedb.IntermediateRoot(false), nil
}
...
func (g *Genesis) ToBlock() *types.Block {
	...
	return types.NewBlock(head, nil, nil, nil, merkle.NewStackTrie(nil)).WithWithdrawals(withdrawals)
}
