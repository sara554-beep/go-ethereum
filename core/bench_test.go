...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	SetupGenesisBlock(db, gspec)
	chain, _ := GenerateChain(gspec.Config, gspec.ToBlock(), ethash.NewFaker(), db, b.N, gen)

	// Time the insertion of the new chain.
	// State and blocks are stored in the same DB.
	chainman, _ := NewBlockChain(db, gspec, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
