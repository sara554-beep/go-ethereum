...
func TestDeriveSha(t *testing.T) {
	...
	for len(txs) < 1000 {
		exp := types.DeriveSha(txs, trie.NewEmpty(trie.NewHashDatabase(rawdb.NewMemoryDatabase())))
...
func TestFuzzDeriveSha(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		seed := rndSeed + i
		exp := types.DeriveSha(newDummy(i), trie.NewEmpty(trie.NewHashDatabase(rawdb.NewMemoryDatabase())))
...
func TestDerivableList(t *testing.T) {
	...
	for i, tc := range tcs[1:] {
		exp := types.DeriveSha(flatList(tc), trie.NewEmpty(trie.NewHashDatabase(rawdb.NewMemoryDatabase())))
