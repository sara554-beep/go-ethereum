...
func testRepair(t *testing.T, tt *rewindTest, snapshots bool) {
	...
	chain, err := NewBlockChain(db, config, genesis, nil, engine, vm.Config{}, nil, nil)
	if err != nil {
	...
	if tt.sidechainBlocks > 0 {
		sideblocks, _ = GenerateChain(params.TestChainConfig, genesis.ToBlock(), engine, rawdb.NewMemoryDatabase(), tt.sidechainBlocks, func(i int, b *BlockGen) {
	...
	canonblocks, _ := GenerateChain(params.TestChainConfig, genesis.ToBlock(), engine, rawdb.NewMemoryDatabase(), tt.canonicalBlocks, func(i int, b *BlockGen) {
	...
	newChain, err := NewBlockChain(db, nil, genesis, nil, engine, vm.Config{}, nil, nil)
	if err != nil {
...
func TestIssue23496(t *testing.T) {
	...
	blocks, _ := GenerateChain(params.TestChainConfig, genesis.ToBlock(), engine, rawdb.NewMemoryDatabase(), 4, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{0x02})
		b.SetDifficulty(big.NewInt(1000000))
	})
