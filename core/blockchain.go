...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	bc.stateCache = state.NewDatabaseWithNodeDB(bc.triedb)
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, root common.Hash, repair bool) (uint64, error) {
	...
	resetGenesis := func() {
		...
		if err := CommitGenesisState(bc.triedb, bc.genesisBlock.Hash()); err != nil {
...
func (bc *BlockChain) Stop() {
	...
	if bc.triedb.Scheme().Name() == trie.PathScheme {
		...
		if !bc.cacheConfig.TrieDirtyDisabled {
			...
			if snapBase != (common.Hash{}) {
				...
				if err := triedb.Commit(snapBase); err != nil {
					log.Error("Failed to commit recent state trie", "err", err)
				}
			}
			for !bc.triegc.Empty() {
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) error {
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		return bc.triedb.Commit(root)
	} else {
		// Full but not archive node, do proper garbage collection
		bc.triedb.Reference(root) // metadata reference to keep trie alive
		bc.triegc.Push(root, -int64(block.NumberU64()))

		if current := block.NumberU64(); current > TriesInMemory {
			// If we exceeded our memory allowance, flush matured singleton nodes to disk
			var (
				nodes = bc.triedb.Size()
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals, setHead bool) (int, error) {
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		stats.report(chain, it.index, bc.triedb.Size(), setHead)
