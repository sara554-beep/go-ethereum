...
type CacheConfig struct {
	...
	StateLimit          uint64        // Number of blocks from head whose state histories are reserved.
...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	bc.triedb = trie.NewDatabase(db, &trie.Config{
		...
		StateLimit: cacheConfig.StateLimit,
	})
	bc.stateCache = state.NewDatabaseWithConfig(bc.triedb)
	...
	if bc.cacheConfig.SnapshotLimit > 0 {
		...
		// TODO(rjl493456442) why to make the change for block number comparison?
		head := bc.CurrentBlock()
			...
			if newHeadBlock == nil {
				...
				for {
					...
					if !bc.StateRecoverable(newHeadBlock.Root()) {
						// TODO if the current block is un-recoverable, then it's meaningless
						// to check further. Please fix/improve it later.
					...
					if beyondRoot || newHeadBlock.NumberU64() == 0 {
						if newHeadBlock.NumberU64() == 0 {
							...
							if !bc.HasState(newHeadBlock.Root()) {
								rawdb.DeleteTrieNode(bc.db, trie.EncodeStorageKey(common.Hash{}, nil))
								if err := CommitGenesisState(bc.db, newHeadBlock.Hash()); err != nil {
						...
						log.Debug("Rewound to block with state", "number", newHeadBlock.NumberU64(), "hash", newHeadBlock.Hash(), "root", newHeadBlock.Root())
						break
					}
...
func (bc *BlockChain) insertSideChain(block *types.Block, it *insertIterator) (int, error) {
	...
	// Check if the state is present or the state is able to recover.
	parent := it.previous()

	// If the parent is un-recoverable, then it's meaningless to check
	// further. TODO(rjl493456442) fix/improve it.
