...
func TestInvalidCliqueConfig(t *testing.T) {
	...
	if _, err := block.Commit(db, trie.NewDatabase(db, &trie.Config{Scheme: trie.HashScheme})); err == nil {
	...
	tests := []struct {
	...
	}{
		{
			...
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, error) {
				return SetupGenesisBlock(db, trie.NewDatabase(db, &trie.Config{Scheme: trie.HashScheme}), new(Genesis))
			},
		...
		},
		{
			...
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, error) {
				return SetupGenesisBlock(db, trie.NewDatabase(db, &trie.Config{Scheme: trie.HashScheme}), nil)
			},
		...
		},
		{
			...
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, error) {
				customg.MustCommit(db)
				return SetupGenesisBlock(db, trie.NewDatabase(db, &trie.Config{Scheme: trie.HashScheme}), DefaultRopstenGenesisBlock())
			},
		...
		},
		{
			...
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, error) {
				...
				return SetupGenesisBlock(db, trie.NewDatabase(db, &trie.Config{Scheme: trie.HashScheme}), &customg)
			},
			wantHash:   customghash,
			wantConfig: customg.Config,
		},
