...
func TestSetupGenesis(t *testing.T) {
	...
	tests := []struct {
		name       string
		fn         func(ethdb.Database) (*params.ChainConfig, *Genesis, common.Hash, error)
	...
	}{
		{
			...
			fn: func(db ethdb.Database) (*params.ChainConfig, *Genesis, common.Hash, error) {
		...
		},
		{
			...
			fn: func(db ethdb.Database) (*params.ChainConfig, *Genesis, common.Hash, error) {
				...
				bc, _ := NewBlockChain(db, &oldcustomg, nil, oldcustomg.Config, ethash.NewFullFaker(), vm.Config{}, nil, nil)
				defer bc.Stop()
