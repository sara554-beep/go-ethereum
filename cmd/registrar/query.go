...
func queryCheckpoint(ctx *cli.Context) error {
	...
	if ctx.GlobalIsSet(checkpointIndexFlag.Name) {
		...
		checkpoint, err := contract.Contract().GetCheckpoint(nil, big.NewInt(index))
		if err != nil {
			return err
		}
		fmt.Printf("Checkpoint %d => %s\n", index, common.Hash(checkpoint).Hex())
	} else {
		index, checkpoint, err := contract.Contract().GetLatestCheckpoint(nil)
		if err != nil {
			return err
		}
		fmt.Printf("Latest checkpoint %d => %s\n", index, common.Hash(checkpoint).Hex())
	}
	return nil
}
