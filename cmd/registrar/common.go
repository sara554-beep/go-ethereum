...
var (
	errInvalidURL      = errors.New("invalid remote node URL")
	errNoSpecifiedAddr = errors.New("no specified registrar contract address")
)
...
func setupContract(ctx *cli.Context, client *ethclient.Client) *registrar.Registrar {
	...
	if ctx.GlobalString(contractAddrFlag.Name) == "" {
		...
		case ctx.GlobalIsSet(utils.TestnetFlag.Name):
			contractAddr = registrar.RegistrarAddr[params.TestnetGenesisHash]
		case ctx.GlobalIsSet(utils.RinkebyFlag.Name):
			contractAddr = registrar.RegistrarAddr[params.RinkebyGenesisHash]
		default:
			contractAddr = registrar.RegistrarAddr[params.MainnetGenesisHash]
		}
	} else {
