...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	if ctx.GlobalString(GCModeFlag.Name) == "archive" {
		if ctx.GlobalUint64(TxLookupLimitFlag.Name) != 0 {
			ctx.GlobalSet(TxLookupLimitFlag.Name, "0")
			log.Warn("Disable transaction unindexing for archive node")
		}
		if ctx.GlobalUint64(StateLimitFlag.Name) != 0 {
			ctx.GlobalSet(StateLimitFlag.Name, "0")
			log.Warn("Record full state history for archive node")
		}
	}
