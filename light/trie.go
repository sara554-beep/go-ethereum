...
func (db *odrDatabase) TrieDB() *triedb.Database {
	return nil
}
...
func (t *odrTrie) do(key []byte, fn func() error) error {
	for {
		var err error
		if t.trie == nil {
			t.trie, err = trie.New(t.id.Root, triedb.New(t.db.backend.Database(), nil))
		}
		if err == nil {
			err = fn()
		}
...
func newNodeIterator(t *odrTrie, startkey []byte) trie.NodeIterator {
	...
	if t.trie == nil {
		it.do(func() error {
			t, err := trie.New(t.id.Root, triedb.New(t.db.backend.Database(), nil))
			if err == nil {
				it.t.trie = t
			}
			return err
