...
func (t *odrTrie) Commit(onleaf trie.LeafCallback) (common.Hash, *trie.NodeSet, error) {
	if t.trie == nil {
...
func (t *odrTrie) do(key []byte, fn func() error) error {
	for {
		...
		if t.trie == nil {
			...
			t.trie, err = trie.New(t.id.Root, owner, t.id.Root, trie.NewDatabase(t.db.backend.Database(), &trie.Config{Legacy: true}))
		}
		if err == nil {
			err = fn()
		}
...
func newNodeIterator(t *odrTrie, startkey []byte) trie.NodeIterator {
	...
	if t.trie == nil {
		it.do(func() error {
			...
			t, err := trie.New(t.id.Root, owner, t.id.Root, trie.NewDatabase(t.db.backend.Database(), &trie.Config{Legacy: true}))
			if err == nil {
				it.t.trie = t
			}
			return err
