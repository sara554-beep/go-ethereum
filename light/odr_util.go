...
func GetUntrustedHeaderByNumber(ctx context.Context, odr OdrBackend, number uint64) (*types.Header, error) {
	r := &ChtRequest{BlockNum: number, ChtNum: number / odr.IndexerConfig().ChtSize, Untrusted: true, Config: odr.IndexerConfig()}
	if err := odr.Retrieve(ctx, r); err != nil {
		return nil, err
	}
	return r.Header, nil
...
// GetBlockLogsByHeader retrieves the logs generated by the transactions included in a
// block given by its hash.
func GetBlockLogsByHeader(ctx context.Context, odr OdrBackend, header *types.Header) ([][]*types.Log, error) {
	// Retrieve the potentially incomplete receipts from disk or network
