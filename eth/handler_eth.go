...
type ethHandler ProtocolManager

func (h *ethHandler) Chain() *core.BlockChain { return h.blockchain }
func (h *ethHandler) TxPool() eth.TxPool      { return h.txpool }

func (h *ethHandler) RunPeer(peer *eth.Peer, handler eth.Handler) error {
	return (*ProtocolManager)(h).runEthPeer(peer, handler)
}
...
func (h *ethHandler) OnBlockAnnounces(peer *eth.Peer, hashes []common.Hash, numbers []uint64) error {
	...
	for i := 0; i < len(hashes); i++ {
		if !h.blockchain.HasBlock(hashes[i], numbers[i]) {
