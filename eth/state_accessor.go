...
// by applying the state reverse diffs. There are two options can be chosen:
//
// - Apply the state reverting to the main database
// - Apply the state reverting to an ephemeral shadowy state database
//
...
func (eth *Ethereum) stateAtTransaction(block *types.Block, txIndex int) (core.Message, vm.BlockContext, *state.StateDB, error) {
	...
	statedb, err := eth.StateAtBlock(parent, nil, true)
	if err != nil {
