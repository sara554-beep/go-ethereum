...
func (eth *Ethereum) hashState(block *types.Block, reexec uint64, base *state.StateDB, readOnly bool, preferDisk bool) (statedb *state.StateDB, release tracers.StateReleaseFunc, err error) {
	...
	if base != nil {
		...
		statedb, database, triedb, report = base, base.Database(), base.Database().TrieDB().(*trie.Database), false
...
func (eth *Ethereum) pathState(block *types.Block, base *state.StateDB) (*state.StateDB, func(), error) {
	...
	if base != nil {
		...
		snap, ok := base.Database().TrieDB().(*trie.DatabaseSnapshot)
		if !ok {
			return statedb, noopReleaser, nil
		}
		snap.Hold()
	...
	sdb, err := state.NewDatabaseWithSnapshot(eth.chainDb, snap)
	if err != nil {
		return nil, nil, err
	}
