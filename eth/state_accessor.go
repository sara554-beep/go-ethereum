...
func (eth *Ethereum) hashState(block *types.Block, reexec uint64, base *state.StateDB, readOnly bool) (statedb *state.StateDB, rel func(), err error) {
	var (
		current  *types.Block
	...
	if readOnly {
		...
		if statedb, err = eth.blockchain.StateAt(block.Root()); err == nil {
			eth.blockchain.TrieDB().Reference(block.Root())
...
func (eth *Ethereum) pathState(block *types.Block, reexec uint64, base *state.StateDB, readOnly bool) (*state.StateDB, func(), error) {
...
func (eth *Ethereum) StateAtBlock(block *types.Block, reexec uint64, base *state.StateDB, readOnly bool) (statedb *state.StateDB, rel func(), err error) {
	if eth.blockchain.TrieDB().Scheme().Name() == trie.HashScheme {
		return eth.hashState(block, reexec, base, readOnly)
	}
	return eth.pathState(block, reexec, base, readOnly)
}
