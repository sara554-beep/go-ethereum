...
func (eth *Ethereum) hashState(block *types.Block, reexec uint64, base *state.StateDB, readOnly bool, preferDisk bool) (statedb *state.StateDB, rel func(), err error) {
	var (
		current  *types.Block
	...
	if readOnly {
		...
		if statedb, err = eth.blockchain.StateAt(block.Root()); err == nil {
			...
			return statedb, func() { eth.blockchain.TrieDB().Dereference(block.Root()) }, nil
		}
	}
	...
	if base != nil {
		...
		statedb, database, triedb, report = base, base.Database(), base.Database().NodeDB().(*trie.Database), false
		...
		// Create an ephemeral trie.Database for isolating the live one. Otherwise
		// the internal junks created by tracing will be persisted into the disk.
		triedb = trie.NewDatabase(eth.chainDb, &trie.Config{Cache: 16, Scheme: trie.HashScheme})
		database = state.NewDatabaseWithNodeDB(triedb)
	...
	if report {
		nodes, _ := triedb.Size()
		log.Info("Historical state regenerated", "block", current.NumberU64(), "elapsed", time.Since(start), "nodes", nodes)
	}
	return statedb, func() { triedb.Dereference(block.Root()) }, nil
}

func (eth *Ethereum) pathState(block *types.Block, base *state.StateDB) (*state.StateDB, func(), error) {
	...
	if base != nil {
		_, _, _, err := eth.blockchain.Processor().Process(block, base, vm.Config{})
		if err != nil {
		...
		statedb, err := state.New(root, base.Database(), nil)
		if err != nil {
			return nil, nil, fmt.Errorf("state reset after block %d failed: %v", block.NumberU64(), err)
		}
		snap, ok := base.Database().NodeDB().(*trie.DatabaseSnapshot)
		if !ok {
			return statedb, noopReleaser, nil
		}
		snap.Hold()
		return statedb, snap.Release, nil
	}
	// Create an isolated state snapshot from the live chain. All
	// the mutations caused later can be erased by invoking release
	// function. Note the snapshot might be unavailable in case the
	// requested state is too old. And it's an expensive operation
	// can take a few minutes depends on how many reverts are required.
	snap, err := eth.BlockChain().TrieDB().GetSnapshot(block.Root())
	if err != nil {
		return nil, nil, err
	}
	sdb, err := state.NewDatabaseWithSnapshot(snap, eth.blockchain.TrieDB().DiskDB())
	if err != nil {
		return nil, nil, err
	}
	state, err := state.New(block.Root(), sdb, nil)
	if err != nil {
		return nil, nil, err
	}
	snap.Hold()
...
//               provided, it would be preferrable to start from a fresh state, if we have it
//               on disk.
func (eth *Ethereum) StateAtBlock(block *types.Block, reexec uint64, base *state.StateDB, readOnly bool, preferDisk bool) (statedb *state.StateDB, rel func(), err error) {
	if eth.blockchain.TrieDB().Scheme().Name() == trie.HashScheme {
		return eth.hashState(block, reexec, base, readOnly, preferDisk)
	}
	return eth.pathState(block, base)
}
...
func (eth *Ethereum) stateAtTransaction(block *types.Block, txIndex int, reexec uint64) (core.Message, vm.BlockContext, *state.StateDB, func(), error) {
	...
	statedb, rel, err := eth.StateAtBlock(parent, reexec, nil, true, false)
	if err != nil {
