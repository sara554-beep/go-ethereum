...
var (
	...
	_, genesis   = core.GenesisBlockForTesting(gendb, testAddress, big.NewInt(1000000000000000))
...
// reassembly.
func makeChain(gendb ethdb.Database, n int, seed byte, parent *types.Block) ([]common.Hash, map[common.Hash]*types.Block) {
...
func testSequentialAnnouncements(t *testing.T, light bool) {
	...
	hashes, blocks := makeChain(copyDB(gendb), targetBlocks, 0, genesis)
...
func testPendingDeduplication(t *testing.T, light bool) {
	...
	hashes, blocks := makeChain(copyDB(gendb), 1, 0, genesis)
...
func TestImportDeduplication(t *testing.T) {
	...
	hashes, blocks := makeChain(copyDB(gendb), 2, 0, genesis)
...
func TestDistantPropagationDiscarding(t *testing.T) {
	...
	hashes, blocks := makeChain(copyDB(gendb), 3*maxQueueDist, 0, genesis)
...
func TestEmptyBlockShortCircuit(t *testing.T) {
	// Create a chain of blocks to import
	hashes, blocks := makeChain(copyDB(gendb), 32, 0, genesis)
...
func TestHashMemoryExhaustionAttack(t *testing.T) {
	...
	attack, _ := makeChain(rawdb.NewMemoryDatabase(), targetBlocks, 0, unknownBlock)
...
func TestBlockMemoryExhaustionAttack(t *testing.T) {
	...
	for i := byte(0); len(attack) < blockLimit+2*maxQueueDist; i++ {
		hashes, blocks := makeChain(rawdb.NewMemoryDatabase(), maxQueueDist-1, i, unknownBlock)
