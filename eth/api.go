...
func (api *DebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]common.Address, error) {
	...
	oldTrie, err := trie.NewStateTrie(startBlock.Root(), common.Hash{}, startBlock.Root(), db.NodeDB())
	if err != nil {
		return nil, err
	}
	newTrie, err := trie.NewStateTrie(endBlock.Root(), common.Hash{}, endBlock.Root(), db.NodeDB())
	if err != nil {
		return nil, err
	}
