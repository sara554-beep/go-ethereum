...
func (api *API) traceChain(start, end *types.Block, config *TraceConfig, closed <-chan interface{}) chan *blockTraceResult {
	...
	go func() {
		...
		for number = start.NumberU64(); number < end.NumberU64(); number++ {
			...
			var preferDisk bool
			if statedb != nil {
				if tdb, ok := statedb.Database().NodeDB().(*trie.Database); ok {
					preferDisk = tdb.Size() > defaultTracechainMemLimit
				}
			}
...
func (api *API) IntermediateRoots(ctx context.Context, hash common.Hash, config *TraceConfig) ([]common.Hash, error) {
	...
	statedb, rel, err := api.backend.StateAtBlock(ctx, parent, reexec, nil, true, false)
	if err != nil {
		return nil, err
	}
	defer rel()
...
func (api *API) TraceCall(ctx context.Context, args ethapi.TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, config *TraceCallConfig) (interface{}, error) {
	...
	statedb, rel, err := api.backend.StateAtBlock(ctx, block, reexec, nil, true, false)
	if err != nil {
		return nil, err
	}
	defer rel()
