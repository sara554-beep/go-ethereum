...
func handle(backend Backend, peer *Peer) error {
	// Start up all the broadcasters and tear them down when done
	...
	// Start inbound message processing until the connection breaks
	for {
...
func handleMessage(backend Backend, peer *Peer) error {
	...
	case peer.version >= 63 && msg.Code == getNodeDataMsg:
		...
		for bytes < softResponseLimit && len(data) < downloader.MaxStateFetch {
			...
			if entry, err := backend.Chain().TrieNode(hash); err == nil {
