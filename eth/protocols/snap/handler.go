...
func ServiceGetAccountRangeQuery(chain *core.BlockChain, req *GetAccountRangePacket) ([]*AccountData, [][]byte) {
	...
	tr, err := trie.New(req.Root, common.Hash{}, req.Root, chain.StateCache().NodeDB())
	if err != nil {
		return nil, nil
	}
...
func ServiceGetStorageRangesQuery(chain *core.BlockChain, req *GetStorageRangesPacket) ([][]*StorageData, [][]byte) {
	...
	for _, account := range req.Accounts {
		...
		if origin != (common.Hash{}) || (abort && len(storage) > 0) {
			...
			accTrie, err := trie.NewStateTrie(req.Root, common.Hash{}, req.Root, chain.StateCache().NodeDB())
			if err != nil {
				return nil, nil
			}
			...
			stTrie, err := trie.NewStateTrie(req.Root, account, acc.Root, chain.StateCache().NodeDB())
			if err != nil {
				return nil, nil
			}
			proof := light.NewNodeSet()
...
func ServiceGetTrieNodesQuery(chain *core.BlockChain, req *GetTrieNodesPacket, start time.Time) ([][]byte, error) {
	...
	triedb := chain.StateCache().NodeDB()

	accTrie, err := trie.NewStateTrie(req.Root, common.Hash{}, req.Root, triedb)
	if err != nil {
	...
	for _, pathset := range req.Paths {
		...
		default:
			...
			stTrie, err := trie.NewStateTrie(req.Root, common.BytesToHash(pathset[0]), common.BytesToHash(account.Root), triedb)
			loads++ // always account database reads, even for failures
			if err != nil {
				break
			}
