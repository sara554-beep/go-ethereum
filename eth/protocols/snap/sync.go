...
func (s *Syncer) loadSyncStatus() {
	...
	if status := rawdb.ReadSnapshotSyncStatus(s.db); status != nil {
		if err := json.Unmarshal(status, &progress); err != nil {
			...
			for _, task := range s.tasks {
				...
				task.genTrie = trie.NewStackTrie(func(hash common.Hash, key []byte, val []byte) {
					s.scheme.WriteTrieNode(task.genBatch, key, hash, val)
				})
				for accountHash, subtasks := range task.SubTasks {
					for _, subtask := range subtasks {
						...
						subtask.genTrie = trie.NewStackTrieWithOwner(func(hash common.Hash, key []byte, val []byte) {
							s.scheme.WriteTrieNode(subtask.genBatch, key, hash, val)
						}, accountHash)
					}
				}
			}
	...
	for i := 0; i < accountConcurrency; i++ {
		...
		s.tasks = append(s.tasks, &accountTask{
			...
			genBatch: batch,
			genTrie: trie.NewStackTrie(func(hash common.Hash, key []byte, val []byte) {
				s.scheme.WriteTrieNode(batch, key, hash, val)
			}),
		})
...
func (s *Syncer) processAccountResponse(res *accountResponse) {
	...
	for i, account := range res.accounts {
		...
		if account.Root != emptyRoot {
			if !s.scheme.HasTrieNode(s.db, trie.EncodeStorageKey(res.hashes[i], nil), account.Root) {
...
func (s *Syncer) processStorageResponse(res *storageResponse) {
	...
	for i, account := range res.accounts {
			...
			if res.subTask == nil && i == len(res.hashes)-1 && res.cont {
				...
				if tasks, ok := res.mainTask.SubTasks[account]; !ok {
					...
					tasks = append(tasks, &storageTask{
						...
						root:     acc.Root,
						genBatch: batch,
						genTrie: trie.NewStackTrieWithOwner(func(hash common.Hash, key []byte, val []byte) {
							s.scheme.WriteTrieNode(batch, key, hash, val)
						}, account),
					})
					for r.Next() {
		...
		if i < len(res.hashes)-1 || res.subTask == nil {
			tr := trie.NewStackTrieWithOwner(func(hash common.Hash, key []byte, val []byte) {
				s.scheme.WriteTrieNode(batch, key, hash, val)
			}, account)
