...
// AncientBatch is a write-only database that commits changes to the associated
// ancient store when Write is called. A batch cannot be used concurrently.
...
// AncientBatcher wraps the NewAncientBatch method of a backing data store.
type AncientBatcher interface {
	// NewAncientBatch creates a write-only database that buffers changes to the
	// ancient store until a final write is called.
