...
type AncientWriter interface {
	...
	// is item_n. The deleted items may not be removed from the ancient store immediately,
	// but only when the accumulated deleted data reach the threshold then will be removed
	// all together. So it's possible that after the system restart these items are accessible
	// again.
