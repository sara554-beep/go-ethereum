...
func (s *Server) serveChain(w http.ResponseWriter, r *http.Request) {
...
func (s *Server) serveChainItem(hash common.Hash) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch shift(&r.URL.Path) {
		case "header":
			// Retrieve the header and attempt to return it
			block, err := s.backends.backend(s.backends.hasBlock(hash)).getBlockByHash(hash)
			if err == nil {
