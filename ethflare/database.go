...
func (diff *diffLayer) flush(db *tileDatabase) (*diffLayer, int, error) {
	...
	complete := diff.state != (common.Hash{})
...
func (diff *diffLayer) has(hash common.Hash) (bool, common.Hash) {
	t, state := diff.get(hash)
	if t == nil {
...
func (db *tileDatabase) insert(hash common.Hash, depth uint8, size common.StorageSize, nodes int, hashes []common.Hash, refs []common.Hash) {
	if db.current == nil {
	...
	if db.nodes > commitThreshold {
		diff, committed, err := db.current.cap(db, commitThreshold)
		if err != nil {
			return
		}
		db.current = diff
		db.nodes -= committed
		log.Info("Commit nodes", "number", committed)
	}
	log.Trace("Insert tile", "hash", hash, "size", size, "nodes", nodes)
}
...
func (db *tileDatabase) commit(root common.Hash) error {
	...
	db.current = nil
	log.Info("Commit state", "root", root, "number", db.nodes, "size", db.totalSize, "nodes", db.totalNodes)
	return nil
}

func (db *tileDatabase) has(hash common.Hash) (bool, common.Hash) {
	if db.current != nil {
		if has, state := db.current.has(hash); has {
			return has, state // state can be empty if it's not complete
		}
	}
	...
	if has && err == nil {
		...
		if head == nil {
			return true, common.BytesToHash(head) // Complete disk layer
		}
		return true, common.Hash{} // Uncomplete disk layer
	}
	return false, common.Hash{} // Not found
}

func (db *tileDatabase) get(hash common.Hash) (*tileInfo, common.Hash) {
	if db.current != nil {
		if tile, state := db.current.get(hash); tile != nil {
