...
type backend interface {
	...
	Update(root common.Hash, parent common.Hash, block uint64, nodes *trienode.MergedNodeSet, states *state.Set) error
...
// Reader returns a reader for accessing trie nodes with provided state root.
...
// ReaderAny is a wrapper of Reader, returning an any instead of reader interface
// because of the golang restriction.
func (db *Database) ReaderAny(blockRoot common.Hash) (any, error) {
	return db.Reader(blockRoot)
}

// Update performs a state transition by committing mutated nodes contained in the
...
func (db *Database) Update(root common.Hash, parent common.Hash, block uint64, nodes *trienode.MergedNodeSet, states *state.Set) error {
	if db.preimages != nil {
...
func (db *Database) Recover(target common.Hash) error {
	...
	return pdb.Recover(target, trie.NewMerkleTrieLoader(db))
}
