...
func TestStateSizeTracking(t *testing.T) {
	expSizeA := 3*(common.HashLength+1) + /* account data */
		...
		2*common.HashLength + 1 + /* storage data of 0xb */
	...
	b := newStates(
		...
		map[common.Hash]map[common.Hash][]byte{
			common.Hash{0xa}: {
				common.Hash{0x1}: {0x11}, // 2*common.HashLength+1
				common.Hash{0x3}: {0x31}, // 2*common.HashLength+1
			},
			common.Hash{0xb}: {
				common.Hash{0x1}: {0x11}, // 2*common.HashLength+1
			},
	...
	mergeSize := expSizeA +
		...
		(2*common.HashLength + 1) /* storage data removal of 0xc */
