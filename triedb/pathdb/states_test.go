...
func TestStateSizeTracking(t *testing.T) {
	expSizeA := 3*(common.HashLength+1) + /* account data */
		common.HashLength + 2*(common.HashLength+1) + /* storage data of 0xa */
		common.HashLength + common.HashLength + 1 + /* storage data of 0xb */
		common.HashLength + common.HashLength + 1 /* storage data of 0xc */
	a := newStates(nil,
		map[common.Hash][]byte{
		...
		map[common.Hash]map[common.Hash][]byte{
			common.Hash{0xa}: {
				common.Hash{0x1}: {0x10}, // common.HashLength+1
				common.Hash{0x2}: {0x20}, // common.HashLength+1
			}, // common.HashLength
			common.Hash{0xb}: {
				common.Hash{0x1}: {0x10}, // common.HashLength+1
			}, // common.HashLength
	...
	expSizeB := 2*common.HashLength + /* destruct set data */
		2*(common.HashLength+1) + /* account data */
		common.HashLength + 2*common.HashLength + 2 + /* storage data of 0xa */
		common.HashLength + common.HashLength + 1 /* storage data of 0xb */
	b := newStates(
		map[common.Hash]struct{}{
		...
		map[common.Hash][]byte{
			common.Hash{0xa}: {0xa1}, // common.HashLength+1
			common.Hash{0xb}: {0xb1}, // common.HashLength+1
		},
		map[common.Hash]map[common.Hash][]byte{
			common.Hash{0xa}: { // common.HashLength
				common.Hash{0x1}: {0x11}, // common.HashLength+1
				common.Hash{0x3}: {0x31}, // common.HashLength+1
			},
			common.Hash{0xb}: { // common.HashLength
				common.Hash{0x1}: {0x11}, // common.HashLength+1
			},
		},
	)
	...
	a.merge(b)
	mergeSize := expSizeA +
		2*common.HashLength - /* destruct set data */
		(common.HashLength + 1) - /* account data removal of 0xc */
		(common.HashLength + common.HashLength + 1) /* storage data removal of 0xc */
