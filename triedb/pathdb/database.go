...
type layer interface {
	...
	update(root common.Hash, id uint64, block uint64, nodes map[common.Hash]map[string]*trienode.Node, states *StateSetWithOrigin) *diffLayer
...
func New(diskdb ethdb.Database, config *Config, isVerkle bool) *Database {
	...
	// Setup the state generator if the initial state is fully completed
	db.setGenerator()
	return db
}
...
// setGenerator loads the state generation progress marker and potentially
// resume the state generation if it's permitted.
func (db *Database) setGenerator() {
	// Set the state snapshot generation progress marker to prevent access
	// to uncovered states.
	valid, generator, root := loadGenerator(db.diskdb)
	if !valid {
		generator = &journalGenerator{
			Marker: []byte{},
		} // rebuild the state snapshot from scratch
	}
	...
	// Snapshot generation won't be permitted either if the database is opened
	// in read-only mode, or the snapshot generation itself is not permitted.
	noBuild := db.readOnly || db.config.SnapshotNoBuild
	dl.generator = newGenerator(db, noBuild, generator.Marker, stats)

	// Short-circuit if state generation is not permitted
