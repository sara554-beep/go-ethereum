...
// nodeLoc is a helpful structure that contains the location where the node or
// state is found, as it's useful for debugging purposes.
type nodeLoc struct {
	loc   string
	depth int
}
...
type readOption struct {
	checkHash  bool
	nodeHasher func([]byte) common.Hash
}
...
func (r *reader) Node(owner common.Hash, path []byte, hash common.Hash) ([]byte, error) {
	...
	if got := r.option.nodeHasher(blob); got != hash {
		switch loc.loc {
		case locCleanCache:
