...
func (t *acctStats) addAccount(address common.Address, modify uint64, descSize, dataSize common.StorageSize) {
	t.totalAccounts += 1
	...
	slices.SortFunc(t.accounts, func(a, b *topAccount) int {
...
func (t *acctStats) Report() {
	for i := 0; i < len(t.accounts); i++ {
		log.Info("Top account",
			"i", i+1, "account", t.accounts[i].address.Hex(), "modify", t.accounts[i].modify,
...
func (t *Traverser) processAccount(stats *acctStats, account common.Address) {
