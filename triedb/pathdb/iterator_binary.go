...
func (dl *diffLayer) initBinaryAccountIterator() Iterator {
	...
	if !ok {
		disk := dl.parent.(*diskLayer)
		l := &binaryIterator{
			a: newDiffAccountIterator(common.Hash{}, dl.states.stateSet, nil),
			b: &binaryIterator{
				a: newDiffAccountIterator(common.Hash{}, disk.buffer.states, nil),
				b: newDiskAccountIterator(disk.db.diskdb, common.Hash{}),
			},
...
func (dl *diskLayer) initBinaryStorageIterator(account common.Hash) Iterator {
