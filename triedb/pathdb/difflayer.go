...
func newDiffLayer(parent layer, root common.Hash, id uint64, block uint64, nodes map[common.Hash]map[string][]byte, states *ethstate.Origin) *diffLayer {
	var (
		size  int64
		count int
	)
...
func (dl *diffLayer) update(root common.Hash, id uint64, block uint64, nodes map[common.Hash]map[string][]byte, states *ethstate.Origin) *diffLayer {
