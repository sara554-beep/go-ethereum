...
type diffLayer struct {
	...
	states *ethstate.Origin                          // Associated state change set for building history
...
func newDiffLayer(parent layer, root common.Hash, id uint64, block uint64, nodes map[common.Hash]map[string]*trienode.Node, states *ethstate.Origin) *diffLayer {
	var (
		size  int64
		count int
	)
...
func (dl *diffLayer) update(root common.Hash, id uint64, block uint64, nodes map[common.Hash]map[string]*trienode.Node, states *ethstate.Origin) *diffLayer {
