...
func (dl *diskLayer) account(hash common.Hash, depth int) ([]byte, error) {
	...
	return rawdb.ReadAccountState(dl.db.diskdb, hash), nil
}
...
func (dl *diskLayer) storage(accountHash, storageHash common.Hash, depth int) ([]byte, error) {
	...
	// Try to retrieve the account from the disk.
	return rawdb.ReadStorageState(dl.db.diskdb, accountHash, storageHash), nil
}
...
func (dl *diskLayer) revert(h *history.History) (*diskLayer, error) {
	...
	if !dl.buffer.empty() {
		...
		for addrHash, blob := range accounts {
			if len(blob) == 0 {
				rawdb.DeleteAccountState(batch, addrHash)
			} else {
				rawdb.WriteAccountState(batch, addrHash, blob)
			}
		}
		for addrHash, storages := range storages {
			for storageHash, blob := range storages {
				if len(blob) == 0 {
					rawdb.DeleteStorageState(batch, addrHash, storageHash)
				} else {
					rawdb.WriteStorageState(batch, addrHash, storageHash, blob)
				}
			}
		}
