...
type generator struct {
	...
	diskdb  ethdb.KeyValueStore // Key-value store containing the snapshot data
	triedb  *Database           // Database for traversing the states via trie
...
func newGenerator(triedb *Database, noBuild bool, progress []byte, stats *generatorStats) *generator {
	if stats == nil {
...
func (g *generator) run(root common.Hash) {
	if g.noBuild {
		log.Info("Snapshot generator is not permitted")
		return
	}
	if g.active {
		g.stop()
...
func generateSnapshot(triedb *Database, root common.Hash) *diskLayer {
	...
	dl.generator = newGenerator(triedb, false, genMarker, stats)
