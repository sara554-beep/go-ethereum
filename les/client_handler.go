...
// clientHandler is responsible for receiving and process all incoming server responses.
...
func newClientHandler(ulcConfig *eth.ULCConfig, backend *LightEthereum) *clientHandler {
	handler := &clientHandler{
		backend: backend,
		closeCh: make(chan struct{}),
	}
	if ulcConfig != nil {
		handler.ulc = newULC(ulcConfig)
		log.Info("Enable ultra light client mode")
	}
	var checkpoint uint64
	if cht, ok := params.TrustedCheckpoints[backend.genesis]; ok {
...
// client in ultra light client mode.
func (h *clientHandler) isULCEnabled() bool {
	if h.ulc == nil || len(h.ulc.trustedKeys) == 0 {
		return false
	}
	return true
}
