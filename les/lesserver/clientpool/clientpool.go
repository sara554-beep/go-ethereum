...
func (c *ClientInfo) Balance(absTime mclock.AbsTime) (uint64, uint64) {
	return c.balanceTracker.getBalance(absTime)
}

func (c *ClientPool) PoSBalance(id enode.ID) (uint64, string) {
	pb := c.ndb.getOrNewPB(id)
	return pb.value, pb.meta
}

func (c *ClientInfo) Freeze() {
	c.peer.FreezeClient()
}
...
func (f *ClientPool) Connect(peer clientPoolPeer, capacity uint64) bool {
	...
	if _, ok := f.connectedMap[id]; ok {
		metrics.ClientRejectedMeter.Mark(1)
	...
	metrics.TotalConnectedGauge.Update(int64(f.connectedCap))
	metrics.ClientConnectedMeter.Mark(1)
...
// setDefaultFactors sets the default price factors applied to subsequently connected clients
func (f *ClientPool) GetDefaultFactors() (PriceFactors, PriceFactors) {
	f.lock.Lock()
	defer f.lock.Unlock()

	return f.defaultPosFactors, f.defaultNegFactors
}

func (f *ClientPool) GetPosBalanceIDs(start, stop enode.ID, maxCount int) []enode.ID {
	return f.ndb.getPosBalanceIDs(start, stop, maxCount)
}
...
func (f *ClientPool) dropClient(e *ClientInfo, now mclock.AbsTime, kick bool) {
	...
	metrics.TotalConnectedGauge.Update(int64(f.connectedCap))
	if kick {
		metrics.ClientKickedMeter.Mark(1)
		log.Debug("Client kicked out", "address", e.address)
		f.removePeer(e.id)
	} else {
		metrics.ClientDisconnectedMeter.Mark(1)
...
func (f *ClientPool) RequestCost(p clientPoolPeer, cost uint64) {
	f.lock.Lock()
	defer f.lock.Unlock()
