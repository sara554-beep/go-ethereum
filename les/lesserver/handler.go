...
import (
	...
	lesmetrics "github.com/ethereum/go-ethereum/les/metrics"
...
func (h *serverHandler) runPeer(version uint, p *p2p.Peer, rw p2p.MsgReadWriter) error {
	peer := newClientPeer(int(version), h.server.config.NetworkId, p, lesmetrics.NewMeteredMsgWriter(rw, int(version)))
	defer peer.close()
	h.wg.Add(1)
	defer h.wg.Done()
...
func (h *serverHandler) handle(p *clientPeer) error {
	...
	lesmetrics.ClientConnectionGauge.Update(int64(h.server.peers.len()))
	...
	defer func() {
		...
		h.server.clientPool.Disconnect(p)
		lesmetrics.ClientConnectionGauge.Update(int64(h.server.peers.len()))
		lesmetrics.ConnectionTimer.Update(time.Duration(mclock.Now() - connectedAt))
	}()
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	if msg.Size > protocol.ProtocolMaxMsgSize {
		lesmetrics.ClientErrorMeter.Mark(1)
	...
	case protocol.GetBlockHeadersMsg:
		...
		if metrics.EnabledExpensive {
			lesmetrics.MiscInHeaderPacketsMeter.Mark(1)
			lesmetrics.MiscInHeaderTrafficMeter.Mark(int64(msg.Size))
			defer func(start time.Time) { lesmetrics.MiscServingTimeHeaderTimer.UpdateSince(start) }(time.Now())
		}
		var req struct {
			ReqID uint64
	...
	case protocol.GetBlockBodiesMsg:
		...
		if metrics.EnabledExpensive {
			lesmetrics.MiscInBodyPacketsMeter.Mark(1)
			lesmetrics.MiscInBodyTrafficMeter.Mark(int64(msg.Size))
			defer func(start time.Time) { lesmetrics.MiscServingTimeBodyTimer.UpdateSince(start) }(time.Now())
		}
		var req struct {
			ReqID  uint64
			Hashes []common.Hash
		...
		if accept(req.ReqID, uint64(reqCnt), protocol.MaxBodyFetch) {
			...
			go func() {
				...
				if metrics.EnabledExpensive {
					lesmetrics.MiscOutBodyPacketsMeter.Mark(1)
					lesmetrics.MiscOutBodyTrafficMeter.Mark(int64(reply.size()))
				}
			}()
		}
	...
	case protocol.GetCodeMsg:
		...
		if metrics.EnabledExpensive {
			lesmetrics.MiscInCodePacketsMeter.Mark(1)
			lesmetrics.MiscInCodeTrafficMeter.Mark(int64(msg.Size))
			defer func(start time.Time) { lesmetrics.MiscServingTimeCodeTimer.UpdateSince(start) }(time.Now())
		}
		var req struct {
			ReqID uint64
			Reqs  []CodeReq
		...
		if accept(req.ReqID, uint64(reqCnt), protocol.MaxCodeFetch) {
			...
			go func() {
				...
				sendResponse(req.ReqID, uint64(reqCnt), reply, task.done())
				if metrics.EnabledExpensive {
					lesmetrics.MiscOutCodePacketsMeter.Mark(1)
					lesmetrics.MiscOutCodeTrafficMeter.Mark(int64(reply.size()))
				}
			}()
		}
	...
	case protocol.GetReceiptsMsg:
		...
		if metrics.EnabledExpensive {
			lesmetrics.MiscInReceiptPacketsMeter.Mark(1)
			lesmetrics.MiscInReceiptTrafficMeter.Mark(int64(msg.Size))
			defer func(start time.Time) { lesmetrics.MiscServingTimeReceiptTimer.UpdateSince(start) }(time.Now())
		}
		var req struct {
			ReqID  uint64
			Hashes []common.Hash
		...
		if accept(req.ReqID, uint64(reqCnt), protocol.MaxReceiptFetch) {
			...
			go func() {
				...
				sendResponse(req.ReqID, uint64(reqCnt), reply, task.done())
				if metrics.EnabledExpensive {
					lesmetrics.MiscOutReceiptPacketsMeter.Mark(1)
					lesmetrics.MiscOutReceiptTrafficMeter.Mark(int64(reply.size()))
				}
			}()
		}
	...
	case protocol.GetProofsV2Msg:
		...
		if metrics.EnabledExpensive {
			lesmetrics.MiscInTrieProofPacketsMeter.Mark(1)
			lesmetrics.MiscInTrieProofTrafficMeter.Mark(int64(msg.Size))
			defer func(start time.Time) { lesmetrics.MiscServingTimeTrieProofTimer.UpdateSince(start) }(time.Now())
		}
		var req struct {
			ReqID uint64
			Reqs  []ProofReq
		...
		if accept(req.ReqID, uint64(reqCnt), protocol.MaxProofsFetch) {
			...
			go func() {
				...
				sendResponse(req.ReqID, uint64(reqCnt), reply, task.done())
				if metrics.EnabledExpensive {
					lesmetrics.MiscOutTrieProofPacketsMeter.Mark(1)
					lesmetrics.MiscOutTrieProofTrafficMeter.Mark(int64(reply.size()))
				}
			}()
		}
	...
	case protocol.GetHelperTrieProofsMsg:
		...
		if metrics.EnabledExpensive {
			lesmetrics.MiscInHelperTriePacketsMeter.Mark(1)
			lesmetrics.MiscInHelperTrieTrafficMeter.Mark(int64(msg.Size))
			defer func(start time.Time) { lesmetrics.MiscServingTimeHelperTrieTimer.UpdateSince(start) }(time.Now())
		}
		var req struct {
			ReqID uint64
			Reqs  []HelperTrieReq
		...
		if accept(req.ReqID, uint64(reqCnt), protocol.MaxHelperTrieProofsFetch) {
			...
			go func() {
				...
				sendResponse(req.ReqID, uint64(reqCnt), reply, task.done())
				if metrics.EnabledExpensive {
					lesmetrics.MiscOutHelperTriePacketsMeter.Mark(1)
					lesmetrics.MiscOutHelperTrieTrafficMeter.Mark(int64(reply.size()))
				}
			}()
		}
	...
	case protocol.SendTxV2Msg:
		...
		if metrics.EnabledExpensive {
			lesmetrics.MiscInTxsPacketsMeter.Mark(1)
			lesmetrics.MiscInTxsTrafficMeter.Mark(int64(msg.Size))
			defer func(start time.Time) { lesmetrics.MiscServingTimeTxTimer.UpdateSince(start) }(time.Now())
		}
		var req struct {
			ReqID uint64
...
func (h *serverHandler) getAuxiliaryHeaders(req HelperTrieReq) []byte {
	if req.Type == 0 && req.AuxReq == 1 && len(req.Key) == 8 {
...
func (h *serverHandler) broadcastHeaders() {
	...
	for {
		...
		case ev := <-headCh:
			...
			var (
				signed         bool
				signedAnnounce protocol.AnnounceData
			)
			announce := protocol.AnnounceData{Hash: hash, Number: number, Td: td, ReorgDepth: reorg}
			for _, p := range peers {
				p := p
