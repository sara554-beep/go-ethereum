...
func newLightFetcher(handler *clientHandler) *lightFetcher {
	f := &lightFetcher{
		handler:        handler,
		chain:          handler.backend.blockchain,
		odr:            handler.backend.odr,
	...
	handler.backend.peers.notify(f)
...
func (f *lightFetcher) isTrustedHash(hash common.Hash) bool {
	...
	return 100*numAgreed/len(f.handler.ulc.trustedKeys) >= f.handler.ulc.minTrustedFraction
}
