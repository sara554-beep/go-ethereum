...
func (dl *downloadTester) FastSyncCommitHead(hash common.Hash) error {
	...
	if block := dl.GetBlockByHash(hash); block != nil {
		_, err := trie.NewStateTrie(block.Root(), common.Hash{}, block.Root(), trie.NewDatabase(dl.stateDb, &trie.Config{Scheme: trie.HashScheme}))
		return err
	}
...
func TestDeliverHeadersHang66Snap(t *testing.T) {
	t.SkipNow()
	testDeliverHeadersHang(t, eth.ETH66, SnapSync)
}
