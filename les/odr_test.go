...
func TestOdrGetBlockHeaderLes1(t *testing.T) { testChtOdr(t, 1, nil) }
func TestOdrGetBlockHeaderLes2(t *testing.T) { testChtOdr(t, 2, nil) }

func odrGetHeader(ctx context.Context, db ethdb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash common.Hash) []byte {
...
func testOdr(t *testing.T, protocol int, expFail uint64, fn odrTestFn) {
	...
	client.peers.Unregister(client.remotePeer.id)
	time.Sleep(time.Millisecond * 10) // ensure that all peerSetNotify callbacks are executed
	test(expFail)
	// expect all retrievals to pass
	client.peers.Register(client.remotePeer)
	time.Sleep(time.Millisecond * 10) // ensure that all peerSetNotify callbacks are executed
	client.peers.lock.Lock()
	client.remotePeer.hasBlock = func(common.Hash, uint64) bool { return true }
