...
func newTestProtocolManager(lightSync bool, blocks int, odr *LesOdr, indexers []*core.ChainIndexer, peers *peerSet, db ethdb.Database) (*ProtocolManager, *backends.SimulatedBackend, error) {
	var (
	...
	reg := newCheckpointRegistrar(db, chain, config, indexConfig, nil, nil, genesis.Hash(), lightSync, exitCh)
	if indexers != nil {
		reg = newCheckpointRegistrar(db, chain, config, indexConfig, indexers[0], indexers[2], genesis.Hash(), lightSync, exitCh)
	}
	pm, err := NewProtocolManager(gspec.Config, indexConfig, lightSync, NetworkId, evmux, peers, chain, nil, db, odr, nil, reg, exitCh, new(sync.WaitGroup))
	if err != nil {
		return nil, nil, err
	}
	// Registrar initialization could failed if checkpoint contract is not deployed.
	if pm.reg != nil {
...
func newTestProtocolManagerMust(t *testing.T, lightSync bool, blocks int, odr *LesOdr, indexers []*core.ChainIndexer, peers *peerSet, db ethdb.Database) (*ProtocolManager, *backends.SimulatedBackend) {
	pm, backend, err := newTestProtocolManager(lightSync, blocks, odr, indexers, peers, db)
	if err != nil {
...
func newTestPeer(t *testing.T, name string, version int, pm *ProtocolManager, checkpoint *light.TrustedCheckpoint, shake bool) (*testPeer, <-chan error) {
	...
	if shake {
		...
		tp.handshake(t, td, head.Hash(), head.Number.Uint64(), genesis.Hash(), checkpoint)
	}
	return tp, errc
}
...
//
// Note regarding the checkpoint:
// 1) when the checkpoint registrar contract has not been deployed(special case)
// checkpoint = nil
// 2) when the number of blocks in the blockchain is less than one checkpoint
// checkpoint = light.EmptyCheckpoint
func (p *testPeer) handshake(t *testing.T, td *big.Int, head common.Hash, headNum uint64, genesis common.Hash, checkpoint *light.TrustedCheckpoint) {
	...
	if checkpoint != nil {
		expList = expList.add("checkpoint", checkpoint)
	}
...
func newServerEnv(t *testing.T, blocks int, protocol int, checkpoint *light.TrustedCheckpoint, waitIndexers func(*core.ChainIndexer, *core.ChainIndexer, *core.ChainIndexer)) (*TestEntity, func()) {
	db := ethdb.NewMemDatabase()
	indexers := testIndexers(db, nil, light.TestServerIndexerConfig)

	pm, b := newTestProtocolManagerMust(t, false, blocks, nil, indexers, nil, db)
	peer, _ := newTestPeer(t, "peer", protocol, pm, checkpoint, true)
...
func newClientServerEnv(t *testing.T, blocks int, protocol int, waitIndexers func(*core.ChainIndexer, *core.ChainIndexer, *core.ChainIndexer), newPeer bool) (*TestEntity, *TestEntity, func()) {
	...
	pm, b := newTestProtocolManagerMust(t, false, blocks, nil, indexers, peers, db)
	lpm, lb := newTestProtocolManagerMust(t, true, 0, odr, lIndexers, lPeers, ldb)
