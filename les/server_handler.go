...
type serverHandler struct {
	...
	isSynced func() bool    // Callback function used to determine whether local node is synced.
}

func newServerHandler(server *LesServer, blockchain *core.BlockChain, chainDb ethdb.Database, txpool *core.TxPool, isSynced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *peer) error {
	// todo(rjl493456442) should we add the connection checking here?
	// free client pool will take care of the connection limitation.

	// Reject light clients if server is not synced.
	if h.isSynced != nil && !h.isSynced() {
...
func (h *serverHandler) handleMsg(p *peer) error {
	...
	accept := func(reqID, reqCnt, maxCnt uint64) bool {
		if reqCnt == 0 || reqCnt > maxCnt {
			return false
		}
		maxCost = p.fcCosts.getCost(msg.Code, reqCnt)
		if accepted, bufShort, priority := p.fcClient.AcceptRequest(reqID, respId, maxCost); !accepted {
			if bufShort > 0 {
				p.Log().Error("Request came too early", "remaining", common.PrettyDuration(time.Duration(bufShort*1000000/p.fcParams.MinRecharge)))
			}
			return false
		} else {
			task = h.server.servingQueue.newTask(priority)
		}
		return task.start()
	}
	...
	case GetCodeMsg:
		...
		go func() {
			for i, req := range req.Reqs {
				...
				number := rawdb.ReadHeaderNumber(h.chainDb, req.BHash)
				if number == nil {
					p.Log().Warn("Failed to retrieve block num for code", "hash", req.BHash)
					continue
				}
				header := rawdb.ReadHeader(h.chainDb, req.BHash, *number)
				if header == nil {
				...
				account, err := h.getAccount(triedb, header.Root, common.BytesToHash(req.AccKey))
				if err != nil {
		...
		go func() {
			...
			for i, req := range req.Reqs {
				...
				if req.BHash != lastBHash {
					...
					if number = rawdb.ReadHeaderNumber(h.chainDb, req.BHash); number == nil {
						p.Log().Warn("Failed to retrieve block num for proof", "hash", req.BHash)
						continue
					}
					if header = rawdb.ReadHeader(h.chainDb, req.BHash, *number); header == nil {
				...
				default:
					...
					account, err := h.getAccount(statedb.TrieDB(), root, common.BytesToHash(req.AccKey))
					if err != nil {
		...
		go func() {
			...
			for i, req := range req.Reqs {
				...
				if auxTrie == nil || req.Type != lastType || req.TrieIdx != lastIdx {
					...
					var prefix string
					if root, prefix = h.getHelperTrie(req.Type, req.TrieIdx); root != (common.Hash{}) {
				...
				if req.AuxReq == auxRoot {
					...
					if req.AuxReq != 0 {
						data := h.getAuxiliaryHeaders(req)
						auxData = append(auxData, data)
						auxBytes += len(data)
					}
				}
