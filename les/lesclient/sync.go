...
func (h *clientHandler) validateCheckpoint(peer *Peer) error {
	...
	cp := peer.Checkpoint
	header, err := light.GetUntrustedHeaderByNumber(ctx, h.backend.odr, peer.CheckpointNumber, peer.ID().String())
	if err != nil {
		return err
	}
	...
	events := h.backend.oracle.LookupCheckpointEvents(logs, cp.SectionIndex, cp.Hash())
	if len(events) == 0 {
...
func (h *clientHandler) synchronise(peer *Peer) {
	...
	case h.backend.oracle == nil || !h.backend.oracle.Running():
		if h.checkpoint == nil {
	...
	if err := h.downloader.Synchronise(peer.ID().String(), peer.Head(), peer.Td(), downloader.LightSync); err != nil {
