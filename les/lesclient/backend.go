...
type LightEthereum struct {
	...
	reqDist    *distributor.RequestDistributor
...
func New(ctx *node.ServiceContext, config *eth.Config) (*LightEthereum, error) {
	...
	leth := &LightEthereum{
		...
		reqDist:        distributor.NewRequestDistributor(&mclock.System{}),
		accountManager: ctx.AccountManager,
		engine:         eth.CreateConsensusEngine(ctx, chainConfig, &config.Ethash, nil, false, chainDb),
		bloomRequests:  make(chan chan *bloombits.Retrieval),
		bloomIndexer:   eth.NewBloomIndexer(chainDb, params.BloomBitsBlocksClient, params.HelperTrieConfirmations),
		serverPool:     serverpool.NewServerPool(chainDb, config.UltraLightServers),
	}
...
func (s *LightEthereum) Protocols() []p2p.Protocol {
	return s.makeProtocols(protocol.ClientProtocolVersions, s.handler.runPeer, func(id enode.ID) interface{} {
		if p := s.peers.ServerPeer(id.String()); p != nil {
			return p.Info()
		}
		return nil
	})
...
func (c *LightEthereum) makeProtocols(versions []uint, runPeer func(version uint, p *p2p.Peer, rw p2p.MsgReadWriter) error, peerInfo func(id enode.ID) interface{}) []p2p.Protocol {
...
func (c *LightEthereum) nodeInfo() interface{} {
...
func (c *LightEthereum) latestLocalCheckpoint() params.TrustedCheckpoint {
...
func (c *LightEthereum) localCheckpoint(index uint64) params.TrustedCheckpoint {
