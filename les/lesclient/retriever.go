...
import (
	...
	"github.com/ethereum/go-ethereum/les/lesclient/serverpool"
...
type sentReq struct {
	...
	sentTo map[distributor.DistPeer]sentReqToPeer

	lastReqQueued bool                 // last request has been queued but not sent
	lastReqSentTo distributor.DistPeer // if not nil then last request has been sent to given peer but not timed out
...
func newRetrieveManager(peers *PeerSet, dist *distributor.RequestDistributor, serverPool peerSelector) *retrieveManager {
...
// cancelled.
func (rm *retrieveManager) retrieve(ctx context.Context, reqID uint64, req *distributor.DistReq, val validatorFunc, shutdown chan struct{}) error {
...
func (rm *retrieveManager) sendReq(reqID uint64, req *distributor.DistReq, val validatorFunc) *sentReq {
	r := &sentReq{
		rm:       rm,
		req:      req,
		id:       reqID,
		sentTo:   make(map[distributor.DistPeer]sentReqToPeer),
	...
	canSend := req.CanSend
	req.CanSend = func(p distributor.DistPeer) bool {
	...
	request := req.Request
	req.Request = func(p distributor.DistPeer) func() {
...
func (rm *retrieveManager) deliver(peer distributor.DistPeer, msg *Msg) error {
	rm.lock.RLock()
	...
	return errResp(protocol.ErrUnexpectedResponse, "reqID = %v", msg.ReqID)
}
...
func (rm *retrieveManager) frozen(peer distributor.DistPeer) {
	rm.lock.RLock()
...
func (r *sentReq) tryRequest() {
	...
	defer func() {
		...
		if hrto {
			...
			if r.rm.peers != nil {
				r.rm.peers.Unregister(pp.ID().String())
			}
		}
...
func (r *sentReq) deliver(peer distributor.DistPeer, msg *Msg) error {
	r.lock.Lock()
	defer r.lock.Unlock()
	...
	if !valid {
		return errResp(protocol.ErrInvalidResponse, "reqID = %v", msg.ReqID)
	}
	return nil
}
...
func (r *sentReq) frozen(peer distributor.DistPeer) {
	r.lock.Lock()
	defer r.lock.Unlock()
