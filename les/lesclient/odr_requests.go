...
func (r *TrieRequest) Request(reqID uint64, peer *Peer) error {
	...
	req := protocol.ProofReq{
		BHash:  r.Id.BlockHash,
		AccKey: r.Id.AccKey,
		Key:    r.Key,
	}
	return peer.RequestProofs(reqID, []protocol.ProofReq{req})
}
...
func (r *CodeRequest) Request(reqID uint64, peer *Peer) error {
	...
	req := protocol.CodeReq{
		BHash:  r.Id.BlockHash,
		AccKey: r.Id.AccKey,
	}
	return peer.RequestCode(reqID, []protocol.CodeReq{req})
}
...
func (r *ChtRequest) CanSend(peer *Peer) bool {
	...
	if r.Untrusted {
		return peer.HeadInfo.Number >= r.BlockNum && peer.ID().String() == r.PeerId
	} else {
...
func (r *ChtRequest) Request(reqID uint64, peer *Peer) error {
	...
	req := protocol.HelperTrieReq{
		Type:    htCanonical,
		TrieIdx: r.ChtNum,
		Key:     encNum[:],
		AuxReq:  auxHeader,
	}
	return peer.RequestHelperTrieProofs(reqID, []protocol.HelperTrieReq{req})
}
...
func (r *BloomRequest) Request(reqID uint64, peer *Peer) error {
	...
	reqs := make([]protocol.HelperTrieReq, len(r.SectionIndexList))
