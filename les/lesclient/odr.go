...
func (odr *LesOdr) Retrieve(ctx context.Context, req light.OdrRequest) (err error) {
	...
	sent := mclock.Now()
	if err = odr.retriever.retrieve(ctx, reqID, rq, func(p distributor.DistPeer, msg *Msg) error { return lreq.Validate(odr.db, msg) }, odr.stop); err == nil {
		// retrieved from network, store in db
		req.StoreResult(odr.db)
		metrics.RequestRTT.Update(time.Duration(mclock.Now() - sent))
	} else {
