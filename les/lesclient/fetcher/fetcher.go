...
type fetcherPeer interface {
	ID() enode.ID
	GetRequestCost(msgcode uint64, amount int) uint64
	QueuedRequest(reqID, maxCost uint64)
	RequestHeadersByHash(reqID uint64, origin common.Hash, amount int, skip int, reverse bool) error
}

type Backend interface {
	RemovePeer(enode.ID)
	ChainDB() ethdb.Database
	TriggerSync(enode.ID)
	//
	// TODO peer subscription system rework
	// SubscribePeers()
}
...
//
// 维护每个Peer最新的announcements信息
type LightFetcher struct {
	ulc     *ULC
	dist    *distributor.RequestDistributor
	backend Backend
	chain   LightChain

	lock            sync.Mutex // lock protects access to the fetcher's internal state variables except sent requests
	maxConfirmedTd  *big.Int
	peers2          map[enode.ID]*fetcherPeerInfo
	peers           map[fetcherPeer]*fetcherPeerInfo
	lastUpdateStats *updateStatsEntry
	syncing         bool
	syncDone        chan fetcherPeer
...
// fetchResponse represents a header download respons
...
func NewLightFetcher(h Backend, chain LightChain, distributor *distributor.RequestDistributor, ulc *ULC) *LightFetcher {
	f := &LightFetcher{
		ulc:            ulc,
		backend:        h,
		dist:           distributor,
		chain:          chain,
		peers2:         make(map[enode.ID]*fetcherPeerInfo),
		deliverChn:     make(chan fetchResponse, 100),
		requested:      make(map[uint64]fetchRequest),
		timeoutChn:     make(chan uint64),
		requestTrigger: make(chan struct{}, 1),
		syncDone:       make(chan fetcherPeer),
	...
	for {
		...
		case reqID := <-f.timeoutChn:
			...
			if ok {
				...
				go f.backend.RemovePeer(req.peer.ID())
			}
		case resp := <-f.deliverChn:
			...
			if !ok || !(f.syncing || f.processResponse(req, resp)) {
				...
				go f.backend.RemovePeer(resp.peer.ID())
			}
			f.lock.Unlock()
		case p := <-f.syncDone:
			f.lock.Lock()
			log.Debug("Done synchronising with peer")
			f.checkSyncedHeaders(p.ID())
			f.syncing = false
			f.lock.Unlock()
...
func (f *LightFetcher) RegisterPeer(p fetcherPeer) {
	// Need a function to set session status
	//p.lock.Lock()
	...
	f.lock.Lock()
	defer f.lock.Unlock()
	f.peers2[p.ID()] = &fetcherPeerInfo{nodeByHash: make(map[common.Hash]*fetcherTreeNode)}
}

// unregisterPeer removes a new peer from the fetcher's peer set
func (f *LightFetcher) UnregisterPeer(p fetcherPeer) {
	f.lock.Lock()
	defer f.lock.Unlock()

	// check for potential timed out block delay statistics
	f.checkUpdateStats(p.ID(), nil)
	delete(f.peers2, p.ID())
}

// announce processes a new announcement message received from a peer, adding new
// nodes to the peer's block tree and removing old nodes if necessary
func (f *LightFetcher) Announce(p fetcherPeer, head *protocol.AnnounceData) {
	f.lock.Lock()
	defer f.lock.Unlock()
	...
	if n != nil {
		...
		for uint64(fp.nodeCnt)+head.Number-n.number > maxNodeCount && fp.root != nil {
			...
			for i, nn := range fp.root.children {
				if rawdb.ReadCanonicalHash(f.backend.ChainDB(), nn.number) == nn.hash {
			...
			fp.root = newRoot
			if newRoot == nil || !f.checkKnownNode(p.ID(), newRoot) {
	...
	f.checkKnownNode(p.ID(), n)
	//p.lock.Lock()
	//p.headInfo = protocol.BlockInfo{Number: head.Number, Hash: head.Hash, Td: head.Td}
	fp.lastAnnounced = n
	//p.lock.Unlock()
	f.checkUpdateStats(p.ID(), nil)
...
func (f *LightFetcher) peerHasBlock(p fetcherPeer, hash common.Hash, number uint64, hasState bool) bool {
	f.lock.Lock()
	defer f.lock.Unlock()
	...
	return rawdb.ReadCanonicalHash(f.backend.ChainDB(), fp.root.number) == fp.root.hash && rawdb.ReadCanonicalHash(f.backend.ChainDB(), number) == hash
}

// requestAmount calculates the amount of headers to be downloaded starting
// from a certain head backwards
func (f *LightFetcher) requestAmount(p enode.ID, n *fetcherTreeNode) uint64 {
	amount := uint64(0)
	nn := n
...
func (f *LightFetcher) nextRequest() (*distributor.DistReq, uint64, bool) {
	var (
...
func (f *LightFetcher) isTrustedHash(hash common.Hash) bool {
	...
	var agreed int
	for peerID, info := range f.peers2 {
		if f.ulc.trusted(peerID) && info.nodeByHash[hash] != nil {
			agreed++
		}
	}
	return 100*agreed/len(f.ulc.keys) >= f.ulc.fraction
}

func (f *LightFetcher) newFetcherDistReqForSync(bestHash common.Hash) *distributor.DistReq {
	return &distributor.DistReq{
...
func (f *LightFetcher) newFetcherDistReq(bestHash common.Hash, reqID uint64, bestAmount uint64) *distributor.DistReq {
	return &distributor.DistReq{
		GetCost: func(dp distributor.DistPeer) uint64 {
			p := dp.(fetcherPeer)
...
func (f *LightFetcher) DeliverHeaders(peer fetcherPeer, reqID uint64, headers []*types.Header) {
...
func (f *LightFetcher) checkSyncedHeaders(p enode.ID) {
	fp := f.peers2[p]
	if fp == nil {
	...
	if f.ulc != nil {
		...
		h, unapproved := f.lastTrustedTreeNode()
...
func (f *LightFetcher) lastTrustedTreeNode() (*types.Header, []common.Hash) {
	...
	commonAncestor := rawdb.FindCommonAncestor(f.backend.ChainDB(), canonical, f.lastTrustedHeader)
...
func (f *LightFetcher) checkKnownNode(p enode.ID, n *fetcherTreeNode) bool {
	if n.known {
		return true
	}
...
func (f *LightFetcher) checkUpdateStats(p enode.ID, newEntry *updateStatsEntry) {
	now := mclock.Now()
	fp := f.peers2[p]
	if fp == nil {
