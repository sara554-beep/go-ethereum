...
// PriceFactors determine the pricing policy (may apply either to positive
// or negative balances which may have different factors).
type PriceFactors struct {
	// TimeFactor is cost unit per nanosecond of connection time
	TimeFactor float64

	// CapacityFactor is cost unit per nanosecond of connection time
	// per 1000000 capacity.
...
//
// NodeBalance implements nodePriority interface.
type NodeBalance struct {
	...
	//
	posFactor PriceFactors
	negFactor PriceFactors

	sumReqCost uint64
	diffTimer  *utils.DiffTimer
	nextUpdate mclock.AbsTime
	initTime   mclock.AbsTime
...
func (n *NodeBalance) activate() {
	...
	n.diffTimer = utils.NewDiffTimer(n.clock)
	n.active = true
	n.bt.switchBalance(n.balance.pos, true)
}
...
func (n *NodeBalance) AddPosBalance(amount int64) (uint64, uint64, error) {
	...
	n.bt.adjustBalance(old, n.balance)
...
func (n *NodeBalance) SetBalance(pos, neg uint64) error {
	...
	n.bt.adjustBalance(n.balance.pos, pb, n.active)
...
func (n *NodeBalance) EstMinPriority(at mclock.AbsTime, capacity uint64, update bool) int64 {
	...
	dt := time.Duration(n.clock.Now() - n.initTime)
	if dt > time.Second {
...
func (n *NodeBalance) GetPriceFactors() (PriceFactors, PriceFactors) {
	n.lock.Lock()
	defer n.lock.Unlock()
...
func (n *NodeBalance) storeBalance(pos, neg bool) {
	...
	if neg {
		n.bt.storeBalance([]byte(n.ipaddr), true, n.balance.neg)
	}
}
...
func (n *NodeBalance) reducedBalance(at mclock.AbsTime, capacity uint64, avgReqCost float64) balance {
	b := n.balance
	n.diffTimer.Diff(at, func(diff mclock.AbsTime) {
		if !b.pos.IsZero() {
			factor := n.posFactor.timePrice(capacity) + n.posFactor.RequestFactor*avgReqCost
			amountDiff := -int64(diff * factor)
			dd := b.pos.Add(amountDiff, n.bt.posExp.LogOffset(at))
			if dd == amountDiff {
				diff = 0
			} else {
				diff += float64(dd) / factor
			}
		}
		if diff > 0 {
			factor := n.negFactor.timePrice(capacity) + n.negFactor.RequestFactor*avgReqCost
			b.neg.Add(int64(diff*factor), n.bt.negExp.LogOffset(at))
		}
	})
	return b
}
