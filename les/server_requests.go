...
func handleGetCode(msg Decoder) (serveRequestFn, uint64, uint64, error) {
	...
	return func(backend serverBackend, p *clientPeer, waitOrStop func() bool) *reply {
		...
		for i, request := range r.Reqs {
			...
			account, err := getAccount(bc.StateCache().TrieDB(), header.Root, common.BytesToHash(request.AccKey))
			if err != nil {
	...
	return func(backend serverBackend, p *clientPeer, waitOrStop func() bool) *reply {
		...
		for i, request := range r.Reqs {
			...
			default:
				...
				account, err := getAccount(backend.BlockChain().StateCache().TrieDB(), root, common.BytesToHash(request.AccKey))
				if err != nil {
