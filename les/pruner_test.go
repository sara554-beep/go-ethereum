...
func TestLightPruner(t *testing.T) {
	...
	// Start light pruner.
	time.Sleep(500 * time.Millisecond) // Ensure light client has finished the indexing
	newPruner(client.chtIndexer, client.bloomTrieIndexer)

	time.Sleep(500 * time.Millisecond) // Ensure pruner have enough time to prune data.
	...
	if err != nil {
		t.Fatalf("Failed to retrieve bloombits of pruned section")
	}
