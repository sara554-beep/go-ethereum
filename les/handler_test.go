...
func testGetProofs(t *testing.T, protocol int) {
	...
	for i := uint64(0); i <= bc.CurrentBlock().NumberU64(); i++ {
		header := bc.GetHeaderByNumber(i)
		trie, _ := trie.New(trie.StateTrieID(header.Root), server.backend.Blockchain().StateCache().TrieDB())
...
func testGetStaleProof(t *testing.T, protocol int) {
	...
	check := func(number uint64, wantOK bool) {
		...
		if wantOK {
			proofsV2 := light.NewNodeSet()
			t, _ := trie.New(trie.StateTrieID(header.Root), server.backend.Blockchain().StateCache().TrieDB())
...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	trie, _ := trie.New(trie.TrieID(root), trie.NewHashDatabase(rawdb.NewTable(server.db, string(rawdb.ChtTablePrefix))))
...
func testGetBloombitsProofs(t *testing.T, protocol int) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		trie, _ := trie.New(trie.TrieID(root), trie.NewHashDatabase(rawdb.NewTable(server.db, string(rawdb.BloomTrieTablePrefix))))
