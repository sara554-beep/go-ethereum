...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	trie, _ := trie.New(trie.TrieID(root), trie.NewHashDatabase(rawdb.NewTable(server.db, light.ChtTablePrefix)))
...
func testGetBloombitsProofs(t *testing.T, protocol int) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		trie, _ := trie.New(trie.TrieID(root), trie.NewHashDatabase(rawdb.NewTable(server.db, light.BloomTrieTablePrefix)))
