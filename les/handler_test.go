...
func testGetProofs(t *testing.T, protocol int) {
	...
	for i := uint64(0); i <= bc.CurrentBlock().NumberU64(); i++ {
		header := bc.GetHeaderByNumber(i)
		trie, _ := trie.New(header.Root, common.Hash{}, header.Root, server.backend.Blockchain().StateCache().NodeDB())
...
func testGetStaleProof(t *testing.T, protocol int) {
	...
	check := func(number uint64, wantOK bool) {
		...
		if wantOK {
			proofsV2 := light.NewNodeSet()
			t, _ := trie.New(header.Root, common.Hash{}, header.Root, server.backend.Blockchain().StateCache().NodeDB())
...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	trie, _ := trie.New(root, common.Hash{}, root, trie.NewDatabase(rawdb.NewTable(server.db, light.ChtTablePrefix), &trie.Config{Scheme: trie.HashScheme}))
...
func testGetBloombitsProofs(t *testing.T, protocol int) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		trie, _ := trie.New(root, common.Hash{}, root, trie.NewDatabase(rawdb.NewTable(server.db, light.BloomTrieTablePrefix), &trie.Config{Scheme: trie.HashScheme}))
