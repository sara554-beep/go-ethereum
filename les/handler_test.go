...
func testGetProofs(t *testing.T, protocol int) {
	...
	for i := uint64(0); i <= bc.CurrentBlock().NumberU64(); i++ {
		header := bc.GetHeaderByNumber(i)
		trie, _ := server.backend.Blockchain().StateCache().OpenTrie(header.Root)
...
func testGetStaleProof(t *testing.T, protocol int) {
	...
	check := func(number uint64, wantOK bool) {
		...
		if wantOK {
			proofsV2 := light.NewNodeSet()
			t, _ := trie.New(header.Root, common.Hash{}, header.Root, server.backend.Blockchain().StateCache().TrieDB())
...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	trie, _ := trie.New(root, common.Hash{}, root, rawdb.NewTable(server.db, light.ChtTablePrefix))
...
func testGetBloombitsProofs(t *testing.T, protocol int) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		trie, _ := trie.New(root, common.Hash{}, root, rawdb.NewTable(server.db, light.BloomTrieTablePrefix))
...
func testTransactionStatus(t *testing.T, protocol int) {
	...
	sdb := state.NewDatabase(server.db)
	gchain, _ := core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), ethash.NewFaker(), sdb, 1, func(i int, block *core.BlockGen) {
		block.AddTx(tx1)
		block.AddTx(tx2)
	})
	...
	gchain, _ = core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), ethash.NewFaker(), sdb, 2, func(i int, block *core.BlockGen) {})
	if _, err := chain.InsertChain(gchain); err != nil {
		panic(err)
	}
