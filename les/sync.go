...
// validateLegacyCheckpoint ensures the given checkpoint belongs to hardcoded
// checkpoint history.
func (pm *ProtocolManager) validateLegacyCheckpoint(cp light.TrustedCheckpoint) bool {
	hardcoded := light.TrustedCheckpoints[pm.blockchain.Genesis().Hash()]
	return cp.HashEqual(hardcoded.Hash())
}
...
func (pm *ProtocolManager) validateCheckpoint(peer *peer) error {
	...
	if pm.validateLegacyCheckpoint(cp) {
		return nil
	}
...
func (pm *ProtocolManager) synchronise(peer *peer) {
	...
	if mode == checkpointSync || mode == legacyCheckpointSync {
		...
		if mode == legacyCheckpointSync {
			if !pm.validateLegacyCheckpoint(cp) {
				log.Debug("Validate checkpoint failed", "reason", "invalid legacy checkpoint")
		...
		log.Debug("Checkpoint syncing start", "peer", peer.id, "checkpoint", cp.SectionIdx)
		...
		ctx, cancel := context.WithTimeout(context.Background(), time.Second*5)
		defer cancel()
		if !pm.blockchain.(*light.LightChain).SyncCheckpoint(ctx, cp) {
