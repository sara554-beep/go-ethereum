...
func New(stack *node.Node, config *ethconfig.Config) (*LightEthereum, error) {
	...
	chainConfig, genesisHash, genesisErr := core.SetupGenesisBlockWithOverride(trie.NewDatabase(chainDb, &trie.Config{Scheme: trie.HashScheme}), config.Genesis, &overrides)
