...
const (
	averageBlockTime   = 15 * time.Second // The average time to find a solution for mining block.
...
type miningWork struct {
	block        *types.Block // Current mining block.
	powHash      common.Hash  // Mining block's no nonce hash.
	seedHash     common.Hash  // Relative seed hash calculated by block number.
	target       common.Hash  // PoW target calculated by block's difficulty.
	createdAt    time.Time    // Created time, used by evicting old work.
	knownAnswers *set.Set     // Set of block hashes known as the correct solution.
}
...
func (ethash *Ethash) remote() {
	...
	getWork := func() ([3]string, error) {
		...
		if work == nil {
			log.Warn("Missing current block's mining environment, it should never occur")
	...
	submitWork := func(nonce types.BlockNonce, mixDigest common.Hash, hash common.Hash) bool {
		...
		// Check duplication to prevent DoS attack.
		header := work.block.Header()
		header.Nonce = nonce
		header.MixDigest = mixDigest
		block := work.block.WithSeal(header)
		if work.knownAnswers.Has(block.Hash()) {
			log.Info("Work submitted but duplicate", "hash", hash)
			return false
		}
		...
		work.knownAnswers.Add(block.Hash())
		return true
	}
	...
	submitStaleSolution := func() {
		...
		if submitted > 0 {
			log.Info("submitted stale solutions", "count", submitted, "remain", len(staleSolutions))
		}
	}
	...
	for {
		...
		case <-ticker.C:
			...
			for hash, work := range works {
				if work.block.HashNoNonce() == currentBlock.HashNoNonce() {
					continue
				}
