...
// here like: (1) accept cheque (2) claim cheque. More complex logic should be
// defined in the wrapper layer based on this.
//
...
func (drawee *ChequeDrawee) waitingReveal() {
	...
	for {
		...
		case retCh := <-drawee.queryCh:
			var cheques []*WrappedCheque
			for revealAt, cheques := range active {
				for _, cheque := range cheques {
					cheques = append(cheques, &WrappedCheque{
